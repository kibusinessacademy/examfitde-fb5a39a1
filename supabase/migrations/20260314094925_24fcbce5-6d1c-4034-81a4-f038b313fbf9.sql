...
CREATE OR REPLACE VIEW public.ops_drift_finder AS
...
SELECT 
  ...
  jsonb_build_object(
    'report_version', cp.integrity_report_version,
    'has_report', cp.integrity_report IS NOT NULL,
    'has_v3_summary', cp.integrity_report->'v3'->'summary' IS NOT NULL,
...
WHERE cp.integrity_report IS NOT NULL
  AND cp.status NOT IN ('archived', 'superseded')
  AND (cp.integrity_report_version IS NULL OR cp.integrity_report_version < 'COURSE_READY_v1.4')
...
CREATE OR REPLACE VIEW public.ops_legacy_package_audit AS
SELECT 
  cp.id AS package_id,
  cp.title,
  cp.status::text AS status,
  cp.track,
  cp.integrity_report_version,
  cp.integrity_passed,
  ...
  CASE 
    WHEN cp.integrity_report_version IS NULL AND cp.integrity_report IS NOT NULL THEN true
    WHEN cp.integrity_report_version IS NOT NULL AND cp.integrity_report_version < 'COURSE_READY_v1.4' THEN true
    ELSE false
  END AS needs_report_refresh,
  CASE
    WHEN (SELECT count(*) FROM exam_questions eq WHERE eq.curriculum_id = cp.curriculum_id) < 250 THEN true
    ELSE false
  END AS needs_production_run
FROM course_packages cp
