...
BEGIN
  ...
  IF p_apply THEN
    ...
    -- Audit pro betroffener product_price (NOT NULL constraint). Ohne aktive prices: anchor row mit nil-uuid wäre invalid → fallback: dummy via gen_random_uuid wäre fake. Stattdessen 1 audit pro aktivem price.
    audit_ins AS (
      ...
      FROM to_archive t
      JOIN product_prices pr ON pr.product_id = t.id
      RETURNING 1
    ),
    -- Deactivate prices on archived products (sonst bleiben sie als active price im SSOT)
