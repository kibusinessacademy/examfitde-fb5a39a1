...
BEGIN
  ...
  LOOP
    -- Skip packages without a course title — can't score them
    ...
    -- Adjusted thresholds: Prio 1 >= 4.0, Prio 2 >= 2.5, Prio 3 >= 1.0
    v_rec_prio := CASE
      WHEN v_overall >= 4.0 THEN 1
      WHEN v_overall >= 2.5 THEN 2
      WHEN v_overall >= 1.0 THEN 3
      ELSE 5
    END;
