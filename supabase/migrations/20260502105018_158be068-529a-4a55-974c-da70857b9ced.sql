-- ============================================================================
-- Control-Lane Watchdog v1.2 — Sub-RPC Signatur-Fixes
-- 
-- Befund:
--   - admin_heal_failed_quality_councils() returnt TABLE, nicht jsonb → ->>'healed' Exception → -1
--   - admin_heal_pending_enqueue_drift hat Pflichtargs (p_package_ids uuid[], p_reason text, p_dry_run boolean)
--     Aufruf ohne Args → function does not exist → -1
-- 
-- Fix:
--   - Sub-Heiler korrekt ansprechen
--   - Exceptions nicht schlucken: Fehlertext in metadata.sub_reports landet
--   - Watchdog returnt jetzt healed/failed/skipped pro Sub-Heiler
-- ============================================================================
...
BEGIN
  -- Skip nur bei wirklich frischer Aktivität (updated_at innerhalb 10min)
  SELECT 
  ...
  IF v_lane_active_processing > 0 THEN
    ...
    RETURN jsonb_build_object(
      'skipped',true,'reason','lane_actively_processing',
  ...
  -- ───── Sub-Heal 1: Failed Quality Councils ─────
  -- Korrekte Signatur: TABLE → COUNT der zurückgegebenen Zeilen
  BEGIN
  ...
  -- ───── Sub-Heal 2: Pending-Enqueue Drift ─────
  -- Korrekte Signatur: (uuid[], text, boolean) — wir liefern alle pending_enqueue-Paket-IDs.
  BEGIN
    SELECT array_agg(DISTINCT package_id) INTO v_pending_enq_pkgs
    FROM public.package_steps
    WHERE status::text = 'pending_enqueue';
  ...
  -- ───── Sub-Heal 3: Queued QCs ohne Job ─────
  FOR v_pkg IN
  ...
  -- ───── Final Audit mit vollständigem Sub-Report ─────
  INSERT INTO public.auto_heal_log(action_type,result_status,target_type,metadata)
  VALUES(
    'control_lane_drift_heal',
    ...
    jsonb_build_object(
      ...
      'sub_reports', jsonb_build_object(
        'failed_quality_councils', jsonb_build_object(
          'healed', v_failed_qc_healed,
          'error', v_failed_qc_error
        ),
        'pending_enqueue_drift', jsonb_build_object(
          'input_pkgs', COALESCE(array_length(v_pending_enq_pkgs,1), 0),
          'result', v_pending_enq_result,
          'error', v_pending_enq_error
        )
      )
    )
  );
