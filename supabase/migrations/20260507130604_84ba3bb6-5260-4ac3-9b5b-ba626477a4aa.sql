...
DECLARE
  v_uid     uuid;
  v_prefix  text := '__smoke_p2b_' || substr(replace(gen_random_uuid()::text,'-',''),1,8) || '_';
  v_a uuid; v_b uuid; v_c uuid; v_d uuid; v_isolated uuid;
  v_r1 jsonb; v_r2 jsonb; v_e1 jsonb; v_e2 jsonb;
  v_orphans_root jsonb; v_orphans jsonb;
  v_failures jsonb := '[]'::jsonb;
  v_pass int := 0; v_fail int := 0;
  v_cond boolean;
BEGIN
  SELECT user_id INTO v_uid FROM public.user_roles WHERE role='admin'::app_role LIMIT 1;
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'smoke requires at least one existing admin in user_roles';
  END IF;
  ...
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T1_register_inserted','ctx',v_r1)); END IF;

  v_r2 := public.admin_register_content_node(v_prefix||'a','Smoke A v2','blog','blog');
  v_cond := v_r2->>'action' = 'updated';
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T2_register_updated','ctx',v_r2)); END IF;
  ...
  v_d        := (public.admin_register_content_node(v_prefix||'d','D','product','product_page')->>'node_id')::uuid;
  ...
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T3_link_inserted','ctx',v_e1)); END IF;

  v_e2 := public.admin_link_content_nodes(v_a, v_b, 'internal_link', 'anchor', 5);
  v_cond := v_e2->>'action' = 'updated';
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T4_link_updated','ctx',v_e2)); END IF;
  ...
  FROM src WHERE o->>'node_slug' LIKE v_prefix || '%';
  ...
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T5_isolated_all_missing','ctx',v_orphans)); END IF;

  v_cond := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_orphans) x WHERE x->>'node_slug' = v_prefix||'a');
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T6_a_fully_wired_not_orphan','ctx',v_orphans)); END IF;
  ...
  IF v_cond THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_failures := v_failures || jsonb_build_array(jsonb_build_object('FAIL','T7_c_inbound_only','ctx',v_orphans)); END IF;
  ...
  RETURN jsonb_build_object('pass', v_pass, 'fail', v_fail, 'failures', v_failures);
