...
BEGIN
  ...
  LOOP
    ...
    v_pool := CASE
      ...
      ELSE 'core'
    END;
    v_curriculum_id := rec.curriculum_id;
    
    INSERT INTO job_queue (package_id, job_type, worker_pool, status, priority, meta, payload, curriculum_id)
    VALUES (
      rec.package_id, v_job_type, v_pool, 'pending', rec.priority,
      jsonb_build_object('source', 'orphan_reconciler', 'step_key', rec.step_key),
      jsonb_build_object('packageId', rec.package_id::text, 'curriculumId', v_curriculum_id::text, 'source', 'orphan_reconciler'),
      v_curriculum_id
    )
