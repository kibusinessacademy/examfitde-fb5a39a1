...
BEGIN
  FOR pkg IN
    SELECT cp.id, cp.title AS pkg_title, cp.build_progress, cp.status,
           cp.track::text AS track_text,
           c.title AS cert_title, c.certification_type::text AS cert_type_text
  ...
  LOOP
    ...
    FOR rule IN SELECT * FROM priority_score_rules WHERE is_active = true
    LOOP
      ...
      IF rule.rule_type = 'cert_type' AND pkg.cert_type_text = rule.pattern THEN
        CASE rule.dimension
          WHEN 'monetization' THEN s_monet := GREATEST(s_monet, rule.weight);
          WHEN 'reuse' THEN s_reuse := GREATEST(s_reuse, rule.weight);
          ELSE NULL;
    ...
    INSERT INTO priority_scores (package_id, demand_score, monetization_score, pipeline_score, reuse_score, overall_score)
