...
CREATE OR REPLACE FUNCTION public.recover_and_reenter_package(
  ...
  p_trigger_source text DEFAULT 'ops_panel',
...
BEGIN
  ...
  -- ARCHITECTURE FIX: Set to 'queued' instead of 'building'.
  -- Only the Runner (via acquire_next_package_lease_v2) may transition
  -- to 'building', because it atomically creates a lease + sets status.
  -- Setting 'building' without a lease creates orphan-deadlocks.
  IF v_eligible THEN
  ...
  VALUES ('recover_and_reenter_package', p_trigger_source, 'course_package', p_package_id::text,
    ...
    CASE WHEN v_reentered THEN 'Requeued for runner handoff' WHEN v_reset_steps > 0 THEN 'Reset steps but not eligible' ELSE 'No steps to reset' END,
    v_now,
    jsonb_build_object(
