...
-- Measurement-only layer for /wissen routes. No query dumps. No secrets.
-- ============================================================
...
WITH cur AS (
  SELECT id, snapshot_at FROM public.semantic_graph_snapshots WHERE status = 'published' LIMIT 1
), graph_routes AS (
  SELECT
  ...
  UNION ALL
  SELECT mr.* FROM metric_routes mr
  ...
  SELECT
    ...
    CASE WHEN COALESCE(sum(impressions) FILTER (WHERE date >= current_date - 27), 0) > 0
      THEN COALESCE(sum(clicks) FILTER (WHERE date >= current_date - 27), 0)::numeric / NULLIF(COALESCE(sum(impressions) FILTER (WHERE date >= current_date - 27), 0), 0)
...
AS $$
  WITH guard AS (
    ...
    FROM (
      ...
      FROM rows WHERE clicks_28d > 0 ORDER BY clicks_28d DESC, impressions_28d DESC LIMIT 10
    ) t
  ), attention_routes AS (
    SELECT COALESCE(jsonb_agg(to_jsonb(t) ORDER BY t.impressions_28d DESC, t.route_path), '[]'::jsonb) AS data
    FROM (
      SELECT route_path, route_kind, route_key, entity_label, search_state, impressions_28d, clicks_28d, ctr_28d, avg_position_28d, recommended_action
      FROM rows WHERE search_state <> 'performing' ORDER BY impressions_28d DESC, route_path LIMIT 25
    ) t
  )
  SELECT CASE
    WHEN NOT (SELECT ok FROM guard) THEN jsonb_build_object('error', 'forbidden')
    ELSE jsonb_build_object('summary', to_jsonb(summary), 'top_routes', (SELECT data FROM top_routes), 'attention_routes', (SELECT data FROM attention_routes))
  END FROM summary;
$$;
...
BEGIN
  ...
  LOOP
    ...
    DO UPDATE SET impressions = EXCLUDED.impressions, clicks = EXCLUDED.clicks, ctr = EXCLUDED.ctr, avg_position = EXCLUDED.avg_position, query_count = EXCLUDED.query_count, imported_at = now(), metadata = EXCLUDED.metadata;
...
AS $$
  WITH guard AS (
    ...
    SELECT * FROM public.v_semantic_route_search_health WHERE route_path = _route_path AND (SELECT ok FROM guard) LIMIT 1
  ), daily AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object('date', date, 'impressions', impressions, 'clicks', clicks, 'ctr', ctr, 'avg_position', avg_position, 'query_count', query_count, 'source', source) ORDER BY date), '[]'::jsonb) AS data
    FROM public.semantic_route_search_metrics
