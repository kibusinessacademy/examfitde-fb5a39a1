...
DECLARE
  ...
  v_score numeric; v_badge text; v_verdict text;
...
BEGIN
  ...
  VALUES (
    'package_' || v_dispatch_kind, p_package_id, 'queued', 7,
    jsonb_build_object(
    ...
    jsonb_build_object('bronze_repair', true, 'attempt', v_attempts + 1, 'enqueue_source', 'bronze_targeted_repair'),
  ...
  VALUES ('admin_bronze_targeted_repair_dispatch','bronze_targeted_repair_dispatched',
          ...
          jsonb_build_object('package_id', p_package_id, 'job_id', v_job_id,
            'kind', v_dispatch_kind, 'attempt', v_attempts + 1, 'score', v_score));
