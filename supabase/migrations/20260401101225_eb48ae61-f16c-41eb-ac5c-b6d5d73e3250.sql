...
BEGIN
  ...
  -- G) TRAP TAGGING — tag ALL untagged approved questions with heuristic trap_type
  WITH untagged AS (
    SELECT eq.id, eq.cognitive_level
    FROM exam_questions eq
    WHERE eq.curriculum_id = p_curriculum_id
      AND eq.status = 'approved'
      AND (eq.trap_type IS NULL OR eq.trap_type = '')
  ),
  tagged AS (
    UPDATE exam_questions eq
    SET trap_type = CASE
      WHEN u.cognitive_level IN ('remember', 'understand') THEN 'misconception'
      WHEN u.cognitive_level IN ('apply', 'analyze') THEN 'typical_error'
      WHEN u.cognitive_level IN ('evaluate', 'create') THEN 'calculation_trap'
      ELSE 'typical_error'
    END,
    is_trap = true
    FROM untagged u
  ...
  v_result := jsonb_build_object(
    ...
    'total_approved', v_total_approved + v_promoted + v_reconciled
  );
