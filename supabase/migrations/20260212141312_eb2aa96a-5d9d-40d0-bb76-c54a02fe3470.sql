...
BEGIN
  ...
  SELECT COUNT(*) INTO v_actual_lessons FROM lessons WHERE course_id = p_course_id;
  ...
  FROM (
    SELECT module_id, competency_id, step_number, COUNT(*) as cnt
    FROM lessons WHERE course_id = p_course_id
    GROUP BY module_id, competency_id, step_number
    HAVING COUNT(*) > 1
  ) dupes;
  ...
  WHERE l.course_id = p_course_id
    AND l.step_number = 4
    AND (l.mini_check_json IS NULL OR l.mini_check_json::TEXT = 'null' OR l.mini_check_json::TEXT = '[]');
  ...
  IF v_oral_question_count < v_oral_target THEN
    IF v_oral_question_count < v_oral_target * 0.5 THEN
      ...
      v_issues := v_issues || jsonb_build_object('type', 'oral_scenarios_insufficient', 'target', v_oral_target, 'actual', v_oral_question_count, 'severity', 'critical');
    ELSE
      v_score := v_score - 5;
      v_warnings := v_warnings || jsonb_build_object('type', 'oral_scenarios_below_target', 'target', v_oral_target, 'actual', v_oral_question_count, 'severity', 'warning');
    END IF;
  END IF;

  -- ========== HANDBOOK ==========
  SELECT COUNT(*), COALESCE(SUM(
    CASE WHEN sections IS NOT NULL THEN jsonb_array_length(sections) ELSE 0 END
  ), 0)
  INTO v_handbook_chapter_count, v_handbook_section_count
