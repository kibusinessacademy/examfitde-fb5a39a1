...
declare
  ...
  v_mapped_step text;
  v_active_count int;
  v_is_incremental_dispatcher boolean;
  v_fanout_cap int;
  v_zero_progress_threshold int;
begin
  v_step_key := coalesce(
    p_payload->>'step_key',
    p_payload->>'step',
  ...
  -- Incremental dispatchers: job types that run multiple times by design
  -- These orchestrate child-work and must not be blocked prematurely
  ...
  -- Dynamic caps based on job type
  ...
  -- For incremental dispatchers: higher threshold + check if step still has open work
  if p_package_id is not null then
