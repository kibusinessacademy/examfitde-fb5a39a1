-- Fix: admin_resolve_repair_strategy_for_package called nicht-existente public.is_admin().
-- Die SSOT Admin-Check-Funktion ist public.is_admin_user(uuid).
-- Wir patchen NUR den Admin-Guard am Anfang, Rest der Logik bleibt 1:1 erhalten.
...
DECLARE
  ...
  v_reason                         text;
BEGIN
  -- FIX: is_admin() → is_admin_user(auth.uid())
  ...
  -- Competencies without exam questions
  ...
  WHERE lf.curriculum_id = v_curriculum_id
    AND NOT EXISTS (
      ...
      WHERE eq.competency_id = c.id
        AND eq.package_id = _package_id
    );
  ...
  RETURN jsonb_build_object(
    ...
    'missing_competencies', COALESCE(array_length(v_competencies_missing_questions,1),0),
