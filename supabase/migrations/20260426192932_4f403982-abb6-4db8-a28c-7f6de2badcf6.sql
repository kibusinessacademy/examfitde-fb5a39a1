...
WITH pkg AS (
  SELECT cp.id AS package_id,
         ...
         cp.created_at,
         cp.updated_at,
         cp.integrity_report   AS pkg_integrity_report,
         cp.integrity_passed   AS pkg_integrity_passed
  ...
  SELECT ps.package_id,
         ...
         count(*) FILTER (WHERE ps.status = 'failed'::step_status)  AS steps_failed,
         count(*) FILTER (WHERE ps.status = ANY (ARRAY['queued'::step_status,'enqueued'::step_status,'running'::step_status])) AS steps_open,
         count(*) FILTER (WHERE ps.status = 'skipped'::step_status) AS steps_skipped,
  ...
  SELECT eq.curriculum_id,
         count(*) FILTER (WHERE eq.status = 'approved'::question_status) AS approved_exam_questions,
         count(*) FILTER (WHERE eq.status = ANY (ARRAY['approved'::question_status,'review'::question_status])) AS usable_exam_questions,
         round(100.0 * count(*) FILTER (WHERE eq.status = 'approved'::question_status AND eq.explanation IS NOT NULL)::numeric / NULLIF(count(*) FILTER (WHERE eq.status = 'approved'::question_status), 0)::numeric, 1) AS explanation_coverage_pct,
         round(100.0 * count(*) FILTER (WHERE eq.status = 'approved'::question_status AND eq.trap_type IS NOT NULL AND eq.trap_type <> '')::numeric / NULLIF(count(*) FILTER (WHERE eq.status = 'approved'::question_status), 0)::numeric, 1) AS trap_coverage_pct
  FROM exam_questions eq
  GROUP BY eq.curriculum_id
), handbook AS (
  SELECT hc.curriculum_id, count(*) AS handbook_chapters
  FROM handbook_chapters hc GROUP BY hc.curriculum_id
), minicheck AS (
  SELECT mq.curriculum_id, count(*) FILTER (WHERE mq.status = 'approved') AS approved_minicheck_questions
  FROM minicheck_questions mq GROUP BY mq.curriculum_id
), lesson_counts AS (
  ...
  FROM ai_tutor_context_index ti GROUP BY ti.package_id
), integrity_step AS (
  SELECT ps.package_id, ps.status::text AS integrity_step_status
  FROM package_steps ps
  WHERE ps.step_key = 'run_integrity_check'
), qc AS (
  SELECT ps.package_id, ps.status::text AS quality_council_status
  FROM package_steps ps WHERE ps.step_key = 'quality_council'
), pub AS (
  SELECT ps.package_id, ps.status::text AS auto_publish_status
  FROM package_steps ps WHERE ps.step_key = 'auto_publish'
), oral_step AS (
  SELECT ps.package_id, ps.status::text AS oral_exam_step_status
  FROM package_steps ps WHERE ps.step_key = 'generate_oral_exam'
), upgrade_latest AS (
  SELECT DISTINCT ON (d.curriculum_id) d.curriculum_id,
    d.package_id,
    d.current_track,
    d.recommended_track,
    d.score,
    d.decision,
    d.reasons,
    d.created_at AS upgrade_decision_at
  FROM course_upgrade_decisions d
...
SELECT
  ...
  pkg.build_progress,
  pkg.priority,
  curr.curriculum_status,
  course.course_status,
  course.is_published,
  pkg.track AS package_track,
  curr.curriculum_track,
  curr.program_type,
  ...
  COALESCE(exam_pool.explanation_coverage_pct, 0::numeric) AS explanation_coverage_pct,
  COALESCE(exam_pool.trap_coverage_pct, 0::numeric)        AS trap_coverage_pct,
  ...
  integrity_step.integrity_step_status,
  -- SSOT FIX: integrity_report + integrity_passed direkt aus course_packages
  pkg.pkg_integrity_report               AS integrity_report,
  COALESCE(pkg.pkg_integrity_passed, false) AS integrity_passed,
  pkg.pkg_integrity_report->'hard_fail_reasons' AS hard_fail_reasons,
  qc.quality_council_status,
  pub.auto_publish_status,
...
LEFT JOIN integrity_step ON integrity_step.package_id = pkg.package_id
