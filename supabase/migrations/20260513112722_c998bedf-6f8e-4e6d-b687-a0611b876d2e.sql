-- Hotfix: fn_guard_obsolete_processing_jobs schreibt 'details' (existiert nicht) → 'metadata'
-- Folge der broken Whitelist-Migration 20260513103805. Pipeline-Stillstand seit ~10:35.
  ...
  THEN
    ...
    IF v_step_key IS NOT NULL THEN
      ...
      IF v_step_status IN ('done','skipped') THEN
        ...
        NEW.last_error := 'OBSOLETE_PROCESSING_PREEMPT: step already '||v_step_status;
        NEW.last_error_code := 'OBSOLETE_PROCESSING_PREEMPT';
        NEW.meta := COALESCE(NEW.meta,'{}'::jsonb)
          || jsonb_build_object('cancelled_by','fn_guard_obsolete_processing_jobs','last_error_kind','preempt','at',now()::text);
      END IF;
    END IF;
  END IF;
...
-- Smoke: insert audit row to confirm column works
INSERT INTO public.auto_heal_log (action_type, target_type, target_id, result_status, metadata)
VALUES ('hotfix_applied','system','fn_guard_obsolete_processing_jobs','ok',
        jsonb_build_object('migration','fix_details_column_drift','fixed_at',now()));