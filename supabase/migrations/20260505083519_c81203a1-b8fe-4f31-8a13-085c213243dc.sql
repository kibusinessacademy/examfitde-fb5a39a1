...
BEGIN
  ...
  LOOP
    ...
    -- ── Block-aware skip (single choke point for bronze + blocked packages) ──
    IF public.fn_is_package_progress_blocked(r.pkg_id) THEN
      INSERT INTO auto_heal_log(action_type, trigger_source, target_type, target_id, result_status, metadata)
      VALUES ('producer_blocked_package_progress','admin_heal_step_job_coupling_v4','package',
              r.pkg_id::text,'skipped',
              jsonb_build_object(
                'producer','admin_heal_step_job_coupling_v4',
                'reason','package_progress_blocked',
                'bronze_locked', public.fn_is_bronze_locked(r.pkg_id),
                'step_key', r.step_key_t));
      package_id := r.pkg_id; step_key := r.step_key_t;
      action := 'skipped'; job_id := NULL; err := 'BLOCKED_PACKAGE_PROGRESS';
      RETURN NEXT;
      CONTINUE;
    END IF;
    ...
    BEGIN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'step_key', r.step_key_t,
          'enqueue_source', 'coupling_heal_v4'
        ),
        'pending',
        100,
