...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════
  -- CLASSIFICATION — PASS-FIRST: fachliche Kriterien VOR Repair-History
  -- ══════════════════════════════════════════════════════════════

  -- 1. HARD_FAIL: No questions and no generation running
  ...
  -- 2. PASS check FIRST — before repair-exhaustion!
  --    If pass criteria are met, the pool is healthy regardless of repair history.
  IF NOT v_hard_fail AND v_gate_status IS NULL THEN
    ...
    IF v_is_pass THEN
      -- Pool meets all pass criteria — return PASS immediately
      v_gate_status := 'PASS';
      v_reason_codes := ARRAY[]::text[];
    END IF;
  END IF;

  -- 3. REPAIR_EXHAUSTED — only if NOT passing
  --    tier1_passed_count=0 must NOT cause HARD_FAIL when pass criteria are met.
  IF NOT v_hard_fail AND v_gate_status IS NULL THEN
    IF v_no_effect_repairs >= 3 AND v_active_repair_jobs = 0 THEN
      v_gate_status := 'HARD_FAIL';
      v_hard_fail := true;
