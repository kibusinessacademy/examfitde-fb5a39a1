...
BEGIN
  ...
  SELECT * INTO v_rec FROM public.quality_intelligence_recommendations
  ...
  LOOP
    SELECT curriculum_id INTO v_curriculum_id
      FROM public.course_packages WHERE id = v_target;
  ...
  UPDATE public.quality_intelligence_recommendations
     SET status = 'applied', enqueued_job_id = v_first_job, updated_at = now()
