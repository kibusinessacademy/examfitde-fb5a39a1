-- Fix 1: admin_resolve_promote_hotloop — step_status hat kein 'processing'
...
BEGIN
  ...
  -- Find packages with hotlooping promote jobs
  SELECT array_agg(DISTINCT package_id)
  INTO v_targets
  FROM (
    SELECT (payload->>'package_id')::uuid AS package_id
    FROM job_queue
    WHERE job_type = 'package_promote_blueprint_variants'
      AND status IN ('queued','running','enqueued','pending_enqueue','batch_pending')
  ...
  FOREACH v_pkg IN ARRAY v_targets LOOP
    ...
    BEGIN
      ...
      WHERE job_type='package_promote_blueprint_variants'
        AND status IN ('queued','running','enqueued','pending_enqueue','batch_pending')
        AND (payload->>'package_id')::uuid = v_pkg;
      ...
      IF NOT p_dry_run THEN
        ...
        -- Reset blueprint_variants step to pending so it can re-run cleanly
...
-- Fix 2: admin_bulk_depublish_hollow — admin_actions hat user_id, nicht performed_by
...
BEGIN
  ...
  FROM (
    ...
    LEFT JOIN exam_questions eq
      ON eq.package_id = cp.id AND eq.status = 'approved'
  ...
  IF NOT p_dry_run THEN
    FOREACH v_pkg IN ARRAY v_targets LOOP
      ...
      EXCEPTION WHEN OTHERS THEN
        -- continue with next package on individual failure
        NULL;
      END;
    END LOOP;
