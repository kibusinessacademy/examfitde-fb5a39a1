...
-- Fix: approved_by column is UUID, so set to NULL for system approvals
-- Also update the governance trigger to allow NULL approved_by when called from RPC
...
DECLARE
  v_count INTEGER;
BEGIN
  -- Temporarily disable the governance trigger for this transaction
