...
BEGIN
  ...
  IF v_curr_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'no curricula available for test');
  END IF;

  INSERT INTO course_packages(id, title, status, curriculum_id, package_key)
  VALUES (gen_random_uuid(), 'PHANTOM_GUARD_SMOKE', 'building', v_curr_id,
  ...
  INSERT INTO package_steps(id, package_id, step_key, status, finished_at, updated_at, meta)
  VALUES (gen_random_uuid(), v_pkg_id, v_step_key, 'done'::step_status,
          now() - interval '5 minutes', now() - interval '5 minutes', '{"ok":"true"}'::jsonb)
  RETURNING id INTO v_step_id;
  UPDATE package_steps SET status='queued'::step_status WHERE id=v_step_id;
  SELECT count(*) INTO v_audit_count FROM auto_heal_log
    WHERE action_type='atomic_enqueue_skipped_recent_finalized_step' AND target_id=v_step_id::text;
...
REVOKE ALL ON FUNCTION public.admin_smoke_phantom_producer_guard_v1() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_smoke_phantom_producer_guard_v1() TO authenticated, service_role;