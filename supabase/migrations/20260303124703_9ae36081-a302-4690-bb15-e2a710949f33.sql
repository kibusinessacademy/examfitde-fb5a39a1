...
-- Track-aware package lease acquisition
-- Accepts optional p_track filter to enable fair scheduling across tracks
...
BEGIN
  ...
  WHERE (
    ...
    -- Track filter: if p_track is provided, only consider packages of that track
    AND (p_track IS NULL OR cp.track = p_track)
  ORDER BY
