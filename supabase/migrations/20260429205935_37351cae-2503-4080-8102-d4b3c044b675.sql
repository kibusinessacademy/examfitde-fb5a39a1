  ...
  IF OLD.status = 'building' AND NEW.status IS DISTINCT FROM 'building' THEN
    ...
    IF v_has_artifacts THEN
      WITH deferred AS (
        ...
        SET status = 'pending',
            ...
            meta = COALESCE(jq.meta, '{}'::jsonb) || jsonb_build_object(
              ...
              'deferred_until', (now() + interval '30 minutes')::text
            )
      ...
      IF v_deferred > 0 THEN
        ...
        VALUES (
          ...
          NEW.id::text,
          'deferred',
          format('Deferred %s tail-step jobs (artifacts present) on transition %s→%s', v_deferred, OLD.status, NEW.status),
          jsonb_build_object('package_id', NEW.id, 'deferred_count', v_deferred, 'old_status', OLD.status, 'new_status', NEW.status, 'gate_class', v_gate_class, 'defer_reason', 'TAIL_STEP_RETRYABLE_WITH_ARTIFACTS')
        );
      END IF;
    END IF;
    ...
    WITH cancelled AS (
      ...
      SET status = 'cancelled',
          ...
          locked_at = NULL,
          locked_by = NULL,
          meta = COALESCE(jq.meta, '{}'::jsonb) || jsonb_build_object('cancel_reason', 'package_exit_building', 'transition_source', 'fn_auto_cancel_jobs_on_package_exit', 'transition_prev_status', jq.status, 'transition_at', now()::text, 'old_pkg_status', OLD.status, 'new_pkg_status', NEW.status)
    ...
    IF v_cancelled > 0 THEN
      ...
      VALUES ('auto_cancel_on_exit', NEW.id, NULL,
              jsonb_build_object('cancelled_count', v_cancelled, 'deferred_tail_count', v_deferred, 'old_status', OLD.status, 'new_status', NEW.status, 'gate_class', v_gate_class, 'has_artifacts', v_has_artifacts));
    END IF;
  END IF;
