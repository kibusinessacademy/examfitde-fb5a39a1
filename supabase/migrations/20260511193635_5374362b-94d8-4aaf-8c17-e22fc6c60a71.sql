...
BEGIN
  ...
  -- B1: Duplicate-Guard für 4 Statuses. Wir nutzen den kanonischen step_key
  -- (damit der Trigger einen registrierten job_type ableitet) und messen
  -- Audit-Hits per Delta. Bestehende done-Step verhindert Guard A nicht, weil
  -- wir keinen alten Step finalisieren — nur Job + neuer Queued-Step.
  FOR v_test_label IN SELECT unnest(ARRAY['queued','pending','processing','retry_scheduled']) LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      v_results := v_results || jsonb_build_object(
        ...
        'skipped_reason', 'status_not_insertable: '||SQLERRM);
    ...
    SELECT count(*) - v_audit_before INTO v_audit_delta FROM auto_heal_log
      WHERE action_type='atomic_enqueue_skipped_recent_duplicate'
        AND metadata->>'package_id' = v_pkg_id::text;
    -- Job-Anzahl: vor INSERT war 1 (unser stub), nach Trigger sollte weiter 1 sein.
