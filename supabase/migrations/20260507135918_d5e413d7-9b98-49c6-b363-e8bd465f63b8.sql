...
BEGIN
  ...
  VALUES (
    ...
    format('inserted=%s skipped=%s errors=%s', v_inserted, v_skipped, jsonb_array_length(v_errors)),
    jsonb_build_object(
      'actor', v_actor,
      'reason', p_reason,
      'requested', v_count,
...
REVOKE ALL ON FUNCTION public.admin_apply_content_graph_edges(jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_apply_content_graph_edges(jsonb, text) TO authenticated, service_role;