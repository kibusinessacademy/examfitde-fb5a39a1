-- Fix: WHERE fs.status <> 'done' blocks SSOT_MISMATCH and INTEGRITY_STALE signals
-- Both require step_status = 'done', so filtering out 'done' sabotages them.
-- Change to WHERE fs.status <> 'skipped' so done steps are included in diagnostics.
...
SELECT
  ...
  CASE
    ...
    WHEN fs.status = 'done' THEN 'HEALTHY'
    ELSE 'OK'
  END AS drift_signal
FROM functional_steps fs
...
COMMENT ON VIEW public.ops_pipeline_step_drift IS
'Pipeline drift diagnostics. Signals: SSOT_MISMATCH_COUNCIL_DONE_BUT_PENDING, INTEGRITY_STALE_AFTER_COUNCIL, LOOP_GUARD_BLOCKED, TRUE_STALL, WAITING_PREREQS, PREMATURE_JOB_DISPATCH, HEALTHY, etc. Includes done steps for council/integrity signal detection.';
