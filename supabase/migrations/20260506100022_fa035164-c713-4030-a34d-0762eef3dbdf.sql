...
RETURNS TABLE(action text, out_package_id uuid, out_persona text)
...
BEGIN
  ...
  LOOP
    ...
    FOREACH v_persona IN ARRAY v_personas LOOP
      ...
      ELSE
        INSERT INTO public.product_persona_overlays AS o (
          package_id, persona_type, hero_kicker, hero_headline, hero_subline,
          primary_cta, usp_items, pain_points, trust_items,
          seo_title, seo_description, active, source
        ) VALUES (
          v_pkg.id, v_persona, v_kicker, v_hero_h, v_hero_s,
          v_cta, v_usps, v_pains, v_trust,
          v_seo_t, v_seo_d, true, 'admin_scaffold_v2'
        )
        ON CONFLICT (package_id, persona_type) DO NOTHING;

        IF FOUND THEN RETURN QUERY SELECT 'inserted'::text, v_pkg.id, v_persona::text;
        ELSE RETURN QUERY SELECT 'skipped_existing'::text, v_pkg.id, v_persona::text;
        END IF;
      END IF;
    END LOOP;
  END LOOP;
