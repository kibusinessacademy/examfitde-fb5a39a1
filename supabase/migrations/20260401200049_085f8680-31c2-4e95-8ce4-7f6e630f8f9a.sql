...
-- Fix false positive: "function" detected as missing table
-- and batch validator read-only tolerance
...
BEGIN
  ...
  -- LAYER 4: SET column in UPDATE (safe loop)
  FOR v_table_ref IN
...
-- Fix contract test: batch validator tolerates read-only
...
BEGIN
  ...
  BEGIN
    ...
    IF v_nba ? 'intent' THEN
      ...
      v_pass_count := v_pass_count + 1;
    ELSE
      v_results := v_results || jsonb_build_object('test', 'nba_return_shape', 'pass', false);
      v_fail_count := v_fail_count + 1;
    END IF;
  ...
  -- T7: check_production_quality
  BEGIN
    v_pq := public.check_production_quality(gen_random_uuid(), gen_random_uuid());
    IF v_pq IS NOT NULL THEN
      v_results := v_results || jsonb_build_object('test', 'prodquality_callable', 'pass', true);
      v_pass_count := v_pass_count + 1;
    ELSE
  ...
  -- T8: VFR not_found
  BEGIN
    v_vfr := public.validate_function_references('nonexistent_xyz_func');
