...
results AS (
  SELECT t.label,
         admin_seo_wave_enqueue_one(t.curriculum_id, t.competency_id, t.package_id, t.intent_key, 'azubi', 5, NULL, 'wave5_skeleton_progression', 5, false) AS r
  FROM targets t
)
INSERT INTO auto_heal_log(action_type, target_type, result_status, metadata)
SELECT 'seo_wave5_live_enqueue', 'system', 'success',
       jsonb_build_object(
         'count', (SELECT COUNT(*) FROM results),
         'enqueued', (SELECT COUNT(*) FROM results WHERE r->>'status'='enqueued'),
         'skipped',  (SELECT COUNT(*) FROM results WHERE r->>'status'<>'enqueued'),
         'results', (SELECT jsonb_agg(jsonb_build_object('label', label, 'status', r->>'status', 'job_id', r->>'job_id', 'idem', r->>'idempotency_key')) FROM results)
       );