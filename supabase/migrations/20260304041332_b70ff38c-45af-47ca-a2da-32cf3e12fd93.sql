...
-- Step 1: Fix the publish guard to enforce integrity_passed
  ...
  IF NEW.status = 'published' AND (OLD.status IS DISTINCT FROM 'published') THEN
    ...
    IF NOT COALESCE(NEW.integrity_passed, false) THEN
      ...
      VALUES (format('Publish blocked (integrity): %s', COALESCE(NEW.title, NEW.id::text)),
              v_reason, 'error', 'pipeline', 'course_package', NEW.id::text);
...
-- Step 2: Temporarily disable immutable guard
ALTER TABLE public.course_packages DISABLE TRIGGER trg_guard_published_immutable;

-- Step 3: Depublish packages without integrity
UPDATE public.course_packages
SET status = 'done',
    published_at = NULL
WHERE status = 'published'
  AND integrity_passed = false;

-- Step 4: Re-enable immutable guard
