...
-- Simpler approach: derive current_step from job_queue + step_job_mapping
CREATE OR REPLACE VIEW v_admin_packages_ssot AS
WITH council_agg AS (
  ...
  -- Find step with active job via step_job_mapping
  SELECT DISTINCT ON (jq.package_id) jq.package_id, sjm.step_key
  FROM job_queue jq
  JOIN step_job_mapping sjm ON jq.job_type = ANY(sjm.job_types)
  WHERE jq.status IN ('processing', 'pending', 'queued')
    AND jq.package_id IS NOT NULL
  ORDER BY jq.package_id,
    CASE jq.status WHEN 'processing' THEN 0 WHEN 'pending' THEN 1 WHEN 'queued' THEN 2 ELSE 3 END,
    jq.created_at
)
SELECT cp.id AS package_id,
  ...
  cp.priority,
  cp.build_progress,
  COALESCE(ast.step_key::text, 
    (SELECT ps2.step_key::text FROM package_steps ps2 
     WHERE ps2.package_id = cp.id AND ps2.status NOT IN ('done', 'skipped') 
     ORDER BY ps2.step_key LIMIT 1)
  ) AS current_step,
  cp.blocked_reason,
  cp.stuck_reason,
  cp.last_progress_at,
