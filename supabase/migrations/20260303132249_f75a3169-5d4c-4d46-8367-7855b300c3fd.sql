...
-- FIX: Make priority guard track-aware to prevent cross-track starvation
-- When p_track is provided, only consider incomplete priority<=10 packages OF THAT TRACK.
-- Without this fix, Elite priority<=10 packages block ALL EXAM_FIRST acquisition.
...
DECLARE
  ...
  v_top30_incomplete int := 0;
  v_effective_status text;
  v_raw_val text;
BEGIN
  -- ── Config (jsonb → text via #>>'{}') ──────────────────────
  BEGIN
  ...
  v_max_slots := greatest(1, coalesce(v_max_slots, 5));
  v_wip_limit := greatest(1, coalesce(v_wip_limit, 5));
  ...
  -- ── Priority guard (TRACK-AWARE to prevent cross-track starvation) ──
  -- When p_track is set, only count top-priority packages OF THAT TRACK.
  -- This prevents Elite priority<=10 packages from blocking EXAM_FIRST entirely.
  IF p_track IS NULL THEN
