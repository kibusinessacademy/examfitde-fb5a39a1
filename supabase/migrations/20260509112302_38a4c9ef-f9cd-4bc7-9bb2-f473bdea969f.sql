CREATE OR REPLACE FUNCTION public.admin_wave_heal_lf_coverage(_dry_run boolean DEFAULT true, _max_packages integer DEFAULT 50, _target_per_lf integer DEFAULT 15)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_plan jsonb;
BEGIN
  -- Allow service_role / internal SQL invocations (auth.uid IS NULL).
  -- Authenticated callers must hold admin role.
  ...
  lf_stats AS (
    SELECT bp.package_id, bp.curriculum_id, lf.id AS lf_id, lf.code AS lf_code,
    ...
    FROM building_pkgs bp JOIN learning_fields lf ON lf.curriculum_id = bp.curriculum_id
  ),
  stats AS (
    SELECT package_id, curriculum_id,
      COUNT(*) AS lf_total,
      ...
      jsonb_agg(jsonb_build_object('lf_id', lf_id, 'lf_code', lf_code,
        'bp_count', bp_count, 'approved', approved_count) ORDER BY lf_code) AS lf_breakdown,
      array_agg(lf_code ORDER BY lf_code) FILTER (WHERE bp_count = 0) AS missing_bp_lfs
    FROM lf_stats GROUP BY package_id, curriculum_id
  ),
  classified AS (
    SELECT *,
      CASE WHEN lfs_with_bp = 0 THEN 'full_seed'
  ...
  to_enqueue AS (
    ...
    ORDER BY CASE c.action WHEN 'full_seed' THEN 0 WHEN 'targeted_seed' THEN 1 ELSE 2 END,
             c.lfs_missing_bp DESC
    LIMIT _max_packages
  )
  SELECT jsonb_agg(jsonb_build_object(
    'package_id', package_id, 'curriculum_id', curriculum_id, 'action', action,
    'lf_total', lf_total, 'lfs_with_bp', lfs_with_bp,
    'lfs_missing_bp', lfs_missing_bp, 'lfs_below_target', lfs_below_target,
    'missing_bp_lfs', COALESCE(missing_bp_lfs, ARRAY[]::text[]),
    'lf_breakdown', lf_breakdown))
  INTO v_plan FROM to_enqueue;

  v_plan := COALESCE(v_plan, '[]'::jsonb);

  IF _dry_run THEN
    PERFORM public.fn_log_guardrail_event('wave_heal_lf_coverage_dryrun',
      jsonb_build_object('run_id', v_run_id, 'count', jsonb_array_length(v_plan)));
    RETURN jsonb_build_object('run_id', v_run_id, 'dry_run', true,
  ...
  PERFORM public.fn_log_guardrail_event('wave_heal_lf_coverage_executed',
    jsonb_build_object('run_id', v_run_id,
      'enqueued', COALESCE(jsonb_array_length(v_enqueued), 0),
      'planned', jsonb_array_length(v_plan)));

  RETURN jsonb_build_object('run_id', v_run_id, 'dry_run', false,
    'planned', jsonb_array_length(v_plan),
    'enqueued', COALESCE(jsonb_array_length(v_enqueued), 0),
    'jobs', COALESCE(v_enqueued, '[]'::jsonb), 'plan', v_plan);
END;
$function$;