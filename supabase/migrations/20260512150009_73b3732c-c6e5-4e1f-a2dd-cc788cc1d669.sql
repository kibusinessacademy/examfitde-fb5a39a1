...
BEGIN
  ...
  FOR v_pkg IN
    ...
    quarantine_class AS (
      SELECT cp.id AS pkg_id, cp.title AS pkg_title, cp.curriculum_id,
             'QUARANTINE_STALE_REAP'::text AS d_class,
             1 AS prio, cp.updated_at
    ...
    reconciler_class AS (
      SELECT cp.id AS pkg_id, cp.title AS pkg_title, cp.curriculum_id,
             'RECONCILER_BRONZE_NO_REPORT'::text AS d_class,
             2 AS prio, cp.updated_at
        FROM course_packages cp
        JOIN gate g ON g.pkg_id = cp.id
  ...
  LOOP
    IF EXISTS (
      ...
      package_id   := v_pkg.pkg_id;
      title        := v_pkg.pkg_title;
      ...
      IF NOT p_dry_run THEN
        ...
        VALUES ('bronze_review_drain_skipped','package', v_pkg.pkg_id::text, 'skipped',
                jsonb_build_object('drain_class', v_pkg.d_class, 'reason','active_integrity_job'),
                auth.uid());
      END IF;
      RETURN NEXT;
      CONTINUE;
    ...
    IF v_recent_cooldown THEN
      ...
      IF NOT p_dry_run THEN
        INSERT INTO auto_heal_log(action_type, target_type, target_id, result_status, metadata, actor_id)
        VALUES ('bronze_review_drain_skipped','package', v_pkg.pkg_id::text, 'skipped',
                jsonb_build_object('drain_class', v_pkg.d_class, 'reason','cooldown_5min'),
                auth.uid());
      END IF;
      RETURN NEXT;
      CONTINUE;
    ...
    IF v_pkg.d_class = 'QUARANTINE_STALE_REAP' THEN
      ...
      VALUES ('bronze_review_drain_quarantine_release','package', v_pkg.pkg_id::text, 'success',
              jsonb_build_object('drain_class', v_pkg.d_class), auth.uid());
