CREATE OR REPLACE FUNCTION public._smoke_cleanup_launch_orders(p_ids uuid[])
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  DELETE FROM entitlements WHERE order_id = ANY(p_ids);
  DELETE FROM learner_course_grants WHERE order_id = ANY(p_ids);
  DELETE FROM conversion_events WHERE (metadata->>'order_id')::uuid = ANY(p_ids);
  DELETE FROM ledger_entries WHERE order_id = ANY(p_ids);
  DELETE FROM invoices WHERE order_id = ANY(p_ids);
  DELETE FROM payments WHERE order_id = ANY(p_ids);
  DELETE FROM order_items WHERE order_id = ANY(p_ids);
  DELETE FROM orders WHERE id = ANY(p_ids);
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'smoke cleanup soft-fail: %', SQLERRM;
END;
$$;
...
BEGIN
  ...
  INSERT INTO order_items(order_id,product_id,description,quantity,unit_amount_net_cents,unit_amount_gross_cents)
  VALUES (v_o_synth,v_prod_with_curr,'smoke',1,1000,1000);

  INSERT INTO orders(buyer_user_id,status,stripe_checkout_session_id,currency,tax_mode,total_cents)
  VALUES (v_uid_no_grant,'paid','cs_live_smoke_'||gen_random_uuid()::text,'eur','gross',1000)
  RETURNING id INTO v_o_no_grant;
  INSERT INTO order_items(order_id,product_id,description,quantity,unit_amount_net_cents,unit_amount_gross_cents)
  VALUES (v_o_no_grant,v_prod_with_curr,'smoke',1,1000,1000);

  INSERT INTO orders(buyer_user_id,status,stripe_checkout_session_id,currency,tax_mode,total_cents)
  VALUES (v_uid_with_grant,'paid','cs_live_smoke_'||gen_random_uuid()::text,'eur','gross',1000)
  RETURNING id INTO v_o_with_grant;
  INSERT INTO order_items(order_id,product_id,description,quantity,unit_amount_net_cents,unit_amount_gross_cents)
  VALUES (v_o_with_grant,v_prod_with_curr,'smoke',1,1000,1000);
  ...
  INSERT INTO order_items(order_id,product_id,description,quantity,unit_amount_net_cents,unit_amount_gross_cents)
  VALUES (v_o_no_curr,v_prod_no_curr,'smoke',1,1000,1000);

  v_ids := ARRAY[v_o_synth,v_o_no_grant,v_o_with_grant,v_o_no_items,v_o_no_curr];
  ...
  v_results := v_results || jsonb_build_object('check','health_smoke_eligible_count_eq_1','pass',v_pass,
    'detail',jsonb_build_object('expected',1,'actual',v_health_count));
  ...
  WHERE (elem->>'order_id')::uuid = ANY(v_ids);
  ...
  PERFORM public._smoke_cleanup_launch_orders(v_ids);
...
EXCEPTION WHEN OTHERS THEN
  BEGIN
    IF v_ids IS NOT NULL THEN PERFORM public._smoke_cleanup_launch_orders(v_ids); END IF;
