...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'classification_mix', (
      ...
      FROM (SELECT classification, COUNT(*) AS cnt FROM classified GROUP BY classification) m
    ),
    'top_pressure', (
  ...
  BEGIN
    PERFORM public.fn_emit_audit(
      'verwaltung_daily_brief_workflow_pressure_view','system',NULL,'ok',
      jsonb_build_object('window_days', _window_days, 'user_id', v_uid, 'role', v_role)
    );
