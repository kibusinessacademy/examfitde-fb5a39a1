-- Heal-Cockpit v8.5 — needs_repair_dispatch Action
-- Klassifiziert Pakete mit blocked_reason='pipeline_repair_required',
-- die noch keinen Repair-Versuch hatten, als auto-dispatchbar.
...
job_agg AS (
  SELECT j.package_id,
         ...
         COALESCE((SELECT jsonb_object_agg(t.job_type, t.cnt)
                   FROM job_open_by_type t
                   WHERE t.package_id = j.package_id), '{}'::jsonb) AS open_jobs_by_type
  FROM job_queue j
...
classified AS (
  SELECT b.*,
    CASE
      WHEN (package_status = 'published'::text OR is_published = true)
           AND deficiency_codes IS NOT NULL
           AND array_length(deficiency_codes, 1) > 0 THEN 'hard_rebuild'::text
      ...
      WHEN release_class = 'release_ok'::text
           AND package_status <> 'published'::text
           AND COALESCE(is_published, false) = false
           AND COALESCE(array_length(deficiency_codes, 1), 0) = 0
           AND blocked_reason IS NULL
           AND active_repair_jobs = 0 THEN 'force_publish'::text
      -- NEU v8.5: Pakete, die explizit auf Repair warten, aber noch nichts dispatched bekommen haben
      WHEN release_class = 'release_block'::text
           AND blocked_reason = 'pipeline_repair_required'::text
           AND pending_jobs = 0
           AND processing_jobs = 0
           AND repair_attempts_proxy = 0
           AND COALESCE(array_length(deficiency_codes, 1), 0) > 0 THEN 'needs_repair_dispatch'::text
      WHEN release_class = 'release_warn'::text
           AND active_repair_jobs = 0
           AND active_reconcile_jobs = 0 THEN 'bulk_reconcile'::text
      WHEN release_class = 'release_block'::text
           AND repair_attempts_proxy = 0
           AND pending_jobs = 0
           AND processing_jobs = 0
           AND blocked_reason IS NULL
           AND COALESCE(last_step_change, package_updated_at) > (now() - '24:00:00'::interval) THEN 'awaiting_pipeline'::text
      WHEN processing_jobs > 0 AND last_processing_at > (now() - '00:20:00'::interval)
           OR pending_jobs > 0 AND package_updated_at > (now() - '00:30:00'::interval) THEN 'monitor'::text
...
SELECT package_id,
       ...
       failed_jobs_24h,
       active_repair_jobs,
       active_reconcile_jobs,
       repair_attempts_proxy,
       exhausted_steps,
       blocked_steps,
       last_processing_at,
       last_step_change,
       ...
       recommended_action,
       CASE
         WHEN recommended_action = 'hard_rebuild'::text THEN 'confirm'::text
         WHEN recommended_action = 'guided_recovery'::text THEN 'modal'::text
         WHEN recommended_action = 'mark_content_gap'::text THEN 'confirm'::text
         WHEN recommended_action = 'force_publish'::text THEN 'auto'::text
         WHEN recommended_action = 'needs_repair_dispatch'::text THEN 'auto'::text
         WHEN recommended_action = 'bulk_reconcile'::text THEN 'auto'::text
         ELSE 'observe'::text
       ...
       array_remove(ARRAY[
         ...
         CASE WHEN recommended_action = 'awaiting_pipeline'::text THEN 'awaiting_pipeline_start=true'::text ELSE NULL END,
         CASE WHEN recommended_action = 'needs_repair_dispatch'::text THEN 'needs_repair_dispatch=true'::text ELSE NULL END
       ], NULL::text) AS recommended_action_reasons,
       LEAST(100, GREATEST(0,
         CASE
           WHEN recommended_action = 'hard_rebuild'::text THEN 95
           WHEN recommended_action = 'guided_recovery'::text THEN 90
           WHEN recommended_action = 'mark_content_gap'::text THEN 85
           WHEN recommended_action = 'needs_repair_dispatch'::text THEN 75
           WHEN recommended_action = 'force_publish'::text THEN 70
           WHEN recommended_action = 'bulk_reconcile'::text THEN 55
           WHEN recommended_action = 'awaiting_pipeline'::text THEN 15
           WHEN recommended_action = 'monitor'::text THEN 20
           ELSE 50
         END +
