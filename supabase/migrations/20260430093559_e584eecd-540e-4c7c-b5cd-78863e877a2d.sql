...
BEGIN
  ...
  SELECT persona_keys INTO v_run_persona_keys FROM public.synth_cohort_runs WHERE id = p_run_id;
  ...
  IF EXISTS (
    ...
    VALUES (
      ...
      'heuristic',
      'IHK-Bezug in Anwenden/Wiederholen/Mini-Check stärken.'
    );
    v_findings_count := v_findings_count + 1;
  ...
  IF EXISTS (
    ...
    VALUES (
      ...
      'Doppelte Lesson-Titel',
      '{}'::jsonb, 'heuristic',
      'Doppelte Lesson-Titel konsolidieren.'
    );
    v_findings_count := v_findings_count + 1;
  END IF;
  ...
  WHERE p.active = true
    AND p.persona_key = ANY(v_run_persona_keys);
