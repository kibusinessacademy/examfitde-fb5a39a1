...
-- Relax Strict Tier Gating: Allow priority tiers up to min_incomplete + 1 
-- when WIP capacity is available (>50% free slots)
CREATE OR REPLACE FUNCTION public.acquire_next_package_lease_v2(p_runner_id text, p_lease_seconds integer DEFAULT 120, p_track text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════
  -- RELAXED PRIORITY TIER GATING
  -- Allow packages up to priority_gate_rank config value.
  -- This replaces the old strict "only min incomplete priority" rule
  -- which starved lower tiers even with massive WIP headroom.
  -- Blocked packages are EXCLUDED from tier calculation.
  -- ══════════════════════════════════════════════════════════════
  ...
  -- Allow up to priority_gate_rank (default 50), but at minimum min_incomplete + 1
