...
-- Fix: exclude 'blocked' from v_top30_incomplete count in acquire_next_package_lease
-- This prevents blocked packages from starving the entire scheduler
...
BEGIN
  ...
  -- Read configurable max concurrent slots
  SELECT coalesce(
  ...
  -- FIX: exclude 'blocked' — blocked packages are not actionable work
  --      and must not starve the scheduler
