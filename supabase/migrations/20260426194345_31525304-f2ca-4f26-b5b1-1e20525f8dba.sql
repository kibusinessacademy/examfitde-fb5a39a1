-- Disambiguate enqueue_job_if_absent overloads by passing p_run_after explicitly,
-- which differentiates the second (default now()) from the first (default NULL).
  ...
  LOOP
    ...
    IF r.primary_blocker = 'INTEGRITY_NEVER_CHECKED' THEN
      ...
      IF p_execute THEN
        PERFORM public.enqueue_job_if_absent(
          p_job_type     => 'package_run_integrity_check'::text,
    ...
    ELSIF r.primary_blocker = 'QUALITY_COUNCIL_PENDING' THEN
      ...
      IF COALESCE(r.integrity_passed,false) = true THEN
        ...
        IF p_execute THEN
          PERFORM public.enqueue_job_if_absent(
            p_job_type     => 'package_quality_council'::text,
            p_package_id   => r.package_id,
            p_priority     => 60,
            p_max_attempts => 5,
            p_run_after    => now(),
