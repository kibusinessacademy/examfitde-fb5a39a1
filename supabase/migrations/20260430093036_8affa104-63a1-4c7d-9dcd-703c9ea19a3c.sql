...
-- Fix: synth_start_run — published packages aus course_packages ziehen
...
BEGIN
  ...
  IF p_package_ids IS NULL OR array_length(p_package_ids, 1) IS NULL THEN
    SELECT array_agg(id) INTO v_pkg_ids FROM public.course_packages WHERE status = 'published';
  ELSE
...
-- Fix: synth_run_heuristic — course_id aus course_packages
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE m.course_id = v_course_id
      AND l.step_type IN ('anwenden','wiederholen','mini_check')
      AND (
  ...
  -- 3) learning_objectives auf Einstieg-Lessons
  IF EXISTS (
    SELECT 1 FROM public.lessons l JOIN public.modules m ON m.id = l.module_id
    WHERE m.course_id = v_course_id AND l.step_type = 'einstieg'
      AND (l.learning_objectives IS NULL OR jsonb_array_length(l.learning_objectives) = 0)
  ) THEN
    INSERT INTO public.synth_didactic_findings
      (run_id, package_id, finding_type, severity, detail, suggested_fix)
    VALUES (
      p_run_id, p_package_id, 'no_learning_objectives', 'warn',
      '{}'::jsonb,
      'Lernziele auf Einstieg-Lessons setzen.'
    );
    v_findings_count := v_findings_count + 1;
    v_didactic_score := GREATEST(0, v_didactic_score - 5);
  END IF;
  ...
  SET packages_done = packages_done + 1,
      total_findings = total_findings + v_findings_count,
      total_sessions_simulated = total_sessions_simulated + (SELECT count(*) FROM public.synth_personas WHERE active = true)
  WHERE id = p_run_id;
...
-- Fix: synth_get_run_summary — title statt label, course_packages statt curriculum_packages
