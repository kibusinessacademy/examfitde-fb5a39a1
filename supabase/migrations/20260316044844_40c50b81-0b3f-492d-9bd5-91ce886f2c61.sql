...
BEGIN
  ...
  -- 2) Read guards (using correct enum values)
  SELECT count(*)
  INTO v_active_jobs
  ...
  SELECT count(*)
  INTO v_blocked_steps
  FROM public.package_steps ps
  WHERE ps.package_id = p_package_id
    AND ps.status = 'blocked';

  SELECT count(*)
  INTO v_running_steps
  FROM public.package_steps ps
  WHERE ps.package_id = p_package_id
    AND ps.status = 'running';

  SELECT count(*)
  INTO v_requeueable_steps
  FROM public.package_steps ps
  WHERE ps.package_id = p_package_id
  ...
  IF v_active_jobs > 0 THEN
    v_result := jsonb_build_object(
      ...
      'reset_steps', 0, 'eligible_for_reentry', false, 'reentered', false,
      'final_status', v_pkg.status, 'reason', 'ACTIVE_JOBS_EXIST'
    );
  ...
  IF v_running_steps > 0 THEN
    v_result := jsonb_build_object(
      'ok', true, 'package_id', p_package_id,
      'reset_steps', 0, 'eligible_for_reentry', false, 'reentered', false,
      'final_status', v_pkg.status, 'reason', 'RUNNING_STEPS_EXIST'
    );
    INSERT INTO public.auto_heal_log (action_type, trigger_source, target_type, target_id, result_status, result_detail, created_at, metadata)
  ...
  IF v_escalation_breaker THEN
    v_result := jsonb_build_object(
      'ok', true, 'package_id', p_package_id,
      'reset_steps', 0, 'eligible_for_reentry', false, 'reentered', false,
      'final_status', 'blocked', 'reason', 'ESCALATION_BREAKER_PRESENT'
    );
    INSERT INTO public.auto_heal_log (action_type, trigger_source, target_type, target_id, result_status, result_detail, created_at, metadata)
  ...
  -- 6) Re-enter package atomically if eligible
  IF v_eligible THEN
    UPDATE public.course_packages cp
    SET status = 'building', updated_at = v_now
  ...
  VALUES (
    ...
    CASE
      WHEN v_reentered THEN 'Recovered steps and re-entered package into building'
      WHEN v_reset_steps > 0 THEN 'Recovered steps but package not eligible for re-entry'
      ELSE 'No steps to reset, checked eligibility'
    END,
    v_now,
    jsonb_build_object(
      'package_status_before', v_pkg.status,
      'package_status_after', v_final_status,
      'reset_steps', v_reset_steps,
      'eligible_for_reentry', v_eligible,
      'reentered', v_reentered,
      'requeueable_steps_after', v_requeueable_steps,
      'blocked_steps', v_blocked_steps,
      'running_steps', v_running_steps,
      'escalation_breaker', v_escalation_breaker,
      'reason', p_reason,
...
'Atomically resets failed/blocked/skipped/timeout package steps and re-enters the package into building when eligible. Uses correct step_status enum values.';
