...
BEGIN
  ...
  FOR v_pkg IN
    SELECT id, title, status, archived FROM public.course_packages WHERE id = ANY(p_package_ids)
  LOOP
    v_step_actions := '[]'::jsonb;
    v_steps_reset := 0;
    ...
    SELECT count(*) INTO v_cancelled_loop_count FROM public.job_queue
     WHERE package_id = v_pkg.id AND status = 'cancelled'
      ...
      LOOP
        ...
        SET status = CASE WHEN v_step.status = 'pending_enqueue' THEN 'pending_enqueue' ELSE 'queued' END,
            ...
            meta = COALESCE(meta, '{}'::jsonb) || jsonb_build_object(
              ...
              'previous_status', v_step.status, 'previous_attempts', v_step.attempts)
        WHERE id = v_step.id;
    ...
    ELSE
      ...
      LOOP
        ...
        v_step_actions := v_step_actions || jsonb_build_object(
          'step_id', v_step.id, 'step_key', v_step.step_key,
          'current_status', v_step.status, 'would_reset', true);
      END LOOP;
    END IF;
