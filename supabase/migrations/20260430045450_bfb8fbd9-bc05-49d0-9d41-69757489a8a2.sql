...
BEGIN
  ...
  FOR r IN SELECT * FROM _drift_candidates LOOP
    v_candidates := v_candidates || jsonb_build_object(
      ...
      'step_status', r.step_status,'step_age_s', r.step_age_s,'approved_q', r.approved_q
    );

    -- Kein step-Eintrag → nudge atomic trigger (legt nächsten fälligen Step an + promotet Pipeline)
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'max_per_run', p_max_per_run,'dry_run', p_dry_run,'checked_at', v_started_at
    ),
    CASE
