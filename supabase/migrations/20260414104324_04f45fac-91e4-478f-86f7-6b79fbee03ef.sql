...
-- ═══════════════════════════════════════════════════════════════
-- PART 1: Fix fn_heal_track_step_drift — proper cancel signing
-- ═══════════════════════════════════════════════════════════════
  ...
  LOOP
    ...
    SET status = 'skipped',
        ...
        meta = COALESCE(meta, '{}'::jsonb) || jsonb_build_object(
          'skip_reason', 'track_drift_not_applicable',
    ...
    -- Cancel linked job if any — WITH PROPER AUDIT SIGNING
    IF v_rec.job_id IS NOT NULL THEN
      ...
      SET status = 'cancelled',
          ...
          locked_by = NULL,
          meta = COALESCE(meta, '{}'::jsonb) || jsonb_build_object(
            'cancel_reason', 'track_step_drift_not_applicable',
...
-- ═══════════════════════════════════════════════════════════════
-- PART 2: Immediate healing — skip steps + cancel open jobs
-- for the 4 affected EXAM_FIRST_PLUS packages
-- ═══════════════════════════════════════════════════════════════

-- 2a: Set generate_learning_content steps to skipped
UPDATE package_steps ps
SET status = 'skipped',
    finished_at = now(),
    updated_at = now(),
    last_error = 'track-drift-heal: learning_content not applicable for EXAM_FIRST_PLUS',
    meta = COALESCE(ps.meta, '{}'::jsonb) || jsonb_build_object(
      'skip_reason', 'track_drift_not_applicable',
      'skipped_by', 'migration_immediate_heal',
      'skipped_at', now()::text,
...
-- 2b: Cancel all open package_generate_learning_content jobs for these packages
UPDATE job_queue jq
SET status = 'cancelled',
    completed_at = now(),
    updated_at = now(),
    ...
    meta = COALESCE(jq.meta, '{}'::jsonb) || jsonb_build_object(
      'cancel_reason', 'track_step_drift_migration_heal',
      'transition_source', 'migration_immediate_heal',
      'transition_prev_status', jq.status,
      'transition_at', now()::text
    )
FROM course_packages cp
WHERE cp.id = jq.package_id
  AND cp.track IN ('EXAM_FIRST', 'EXAM_FIRST_PLUS')
