-- Strict read-only reproducer probe for fn_auto_trigger_quality_gate_heal
-- Mirrors the healer's plan-evaluation 1:1 but performs ZERO writes to
-- job_queue / package_steps / course_packages. Only writes ONE audit row.
...
BEGIN
  ...
  IF v_pkg.package_id IS NULL THEN
    ...
    v_skipped_reason := 'package_not_found';
    v_audit_meta := jsonb_build_object(
      'package_id', p_package_id,
      'plan_count', 0,
      'plan_entries', '[]'::jsonb,
      'would_attempt_insert', false,
  ...
  -- ── 2. Mirror outer iteration filter (v_quality_gate_blocked_packages, minutes >= 60) ──
  ...
  -- Healer picks the first entry by priority NULLS LAST, LIMIT 1
  SELECT value
  INTO v_first_plan
  ...
  ELSIF v_plan_count = 0 OR v_first_plan IS NULL OR v_planned_job_type IS NULL THEN
    -- This is the EMPTY-PLAN-FALLTHROUGH branch in fn_auto_trigger_quality_gate_heal
    v_result_status := 'no_plan';
    v_skipped_reason := 'empty_enqueue_plan_would_log_healed_with_leaked_v_job_type';
    v_would_attempt := false;
  ELSE
    v_would_attempt := true;
    -- ── 5. Static trigger-risk prediction (no execution) ────────────
    -- Pure name-based heuristic; flags BEFORE-INSERT guards likely to gate this job_type.
    SELECT jsonb_agg(jsonb_build_object(
      'trigger', t.tgname,
      'function', p.proname,
      'likely_relevant', CASE
        WHEN p.proname IN (
          'fn_guard_job_type_registry',
          'fn_job_queue_ssot_validate',
          'fn_guard_dag_prerequisites',
          'fn_guard_phantom_repair_enqueue',
          'fn_guard_orphan_heal_requires_building',
          'fn_guard_non_building_enqueue_loop',
          'fn_guard_redundant_content_step_enqueue',
          'fn_guard_redundant_seeding',
          'fn_guard_autoheal_duplicate',
          ...
          'fn_guard_ssot_applicability',
          'fn_guard_lease_expired_auto_clear',
          'guard_job_payload',
          'trg_block_publish_without_product',
          'trg_block_publish_enqueue_without_pricing',
    ...
    WHERE c.relname = 'job_queue'
      AND nc.nspname = 'public'
      AND NOT t.tgisinternal
      AND (t.tgtype & 2)::int = 2  -- BEFORE
      AND (t.tgtype & 4)::int = 4  -- INSERT
      AND t.tgenabled <> 'D';

    -- Heuristic refinement on top of static list
    IF EXISTS (
      SELECT 1 FROM public.course_packages
      WHERE package_id = p_package_id
        AND COALESCE((feature_flags->'bronze'->>'locked')::boolean, false) = true
    ) THEN
      v_skipped_reason := 'bronze_locked_package_repair_exam_pool_quality_NOT_in_bronze_guard_list';
      -- repair_exam_pool_quality is NOT in the bronze guard list — this is informational only
    END IF;

    v_result_status := 'would_attempt_insert';
  END IF;

  -- ── 6. Audit (single write) ───────────────────────────────────────
  v_audit_meta := jsonb_build_object(
    'package_id', p_package_id,
    'package_title', v_pkg.title,
    'package_status', v_pkg.status,
    'in_blocked_view', v_in_blocked_view,
    'minutes_since_report', v_minutes_since_report,
    'eligible_for_auto_heal', v_eligible,
    'plan_count', v_plan_count,
    'plan_entries', v_plan,
    'first_plan_picked', v_first_plan,
    'planned_job_type', v_planned_job_type,
    'would_attempt_insert', v_would_attempt,
    'skipped_reason', v_skipped_reason,
...
REVOKE ALL ON FUNCTION public.admin_quality_gate_auto_heal_reproducer_probe(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_quality_gate_auto_heal_reproducer_probe(uuid) TO service_role;

COMMENT ON FUNCTION public.admin_quality_gate_auto_heal_reproducer_probe(uuid) IS
'Strict read-only reproducer for fn_auto_trigger_quality_gate_heal. ZERO writes to job_queue/package_steps/course_packages. Single audit row in auto_heal_log (action_type=quality_gate_auto_heal_reproducer_probe). Admin-only via has_role gate. Result: would_attempt_insert | plan_branch_skipped | no_plan.';

-- Smoke-test: probe runs and writes exactly 1 audit row (will be executed by user via separate read query)
-- Rollback hint: DROP FUNCTION public.admin_quality_gate_auto_heal_reproducer_probe(uuid);