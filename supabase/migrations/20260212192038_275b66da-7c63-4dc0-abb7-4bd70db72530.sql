...
-- Fix validate_course_integrity_v2: count exam via curriculum_id directly (not only via competency JOIN)
-- Also: gate handbook on sections (not just chapters), add structural health info
...
BEGIN
  ...
  -- ========== EXAM ==========
  -- Primary count: via curriculum_id directly (catches all questions including those with NULL competency_id)
  ...
  -- Gate on SECTIONS (not just chapters) - sections are the real content
