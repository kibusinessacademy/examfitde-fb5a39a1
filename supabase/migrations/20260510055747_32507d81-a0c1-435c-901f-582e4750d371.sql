...
BEGIN
  ...
  v_action := 'package_generate_lesson_minichecks';
  ...
  LOOP
    v_skip_reason := NULL; v_job_id := NULL;
    ...
    ELSE
      ...
      IF v_active > 0 THEN v_skip_reason := 'active_jobs_exist'; END IF;
    END IF;
    ...
    IF v_skip_reason IS NULL THEN
      ...
      IF NOT p_dry_run THEN
        INSERT INTO public.job_queue
          (job_type, job_name, package_id, payload, status, priority, max_attempts, run_after, scheduled_at, meta)
        VALUES (
          v_action, COALESCE(v_job_name,'Package Generate Lesson Minichecks'), r.package_id,
          jsonb_build_object(
          ...
          'pending', 50, 3, now(), now(),
          jsonb_build_object(
            'wave', 'soft_drift_mc',
            'enqueue_source', 'soft_drift_mc_required_repair',
            'previous_mc_approval_pct', r.mc_approval_pct,
            'mc_total', r.mc_total,
    ...
    VALUES ('soft_drift_mc_required_repair', r.package_id, 'package',
      CASE WHEN p_dry_run THEN 'dry_run'
           WHEN v_skip_reason IS NULL THEN 'enqueued'
           ELSE 'skipped' END,
      jsonb_build_object('package_id', r.package_id, 'package_title', r.package_title, 'track', r.track,
        ...
        'risk_score', r.risk_score, 'dry_run', p_dry_run,
        'action', CASE WHEN v_skip_reason IS NULL THEN v_action ELSE NULL END,
        'job_id', v_job_id, 'skip_reason', v_skip_reason,
        'mode', 'soft_drift_targeted_mc_repair'));
  ...
  RETURN jsonb_build_object('ok', true, 'dry_run', p_dry_run,
    'job_type', v_action, 'mode', 'soft_drift_targeted_mc_repair',
