-- Disambiguation wrapper: a single, unambiguous enqueue helper
-- that internally picks one of the two enqueue_job_if_absent overloads.
...
AS $$
BEGIN
  -- Use positional call to the 6-arg overload:
  --   (p_job_type text, p_package_id uuid, p_priority int, p_max_attempts int, p_run_after timestamptz, p_payload jsonb)
  PERFORM public.enqueue_job_if_absent(
    p_job_type, p_package_id, p_priority, 5, now(), p_payload
  );
END;
$$;

-- Now patch admin_targeted_blocker_recheck to use the wrapper
...
BEGIN
  ...
  LOOP
    IF r.primary_blocker = 'INTEGRITY_NEVER_CHECKED' THEN
      ...
      executed := false;
      IF p_execute THEN
        PERFORM public._admin_recheck_enqueue(
...
GRANT EXECUTE ON FUNCTION public.admin_targeted_blocker_recheck(boolean) TO anon, authenticated, service_role;
