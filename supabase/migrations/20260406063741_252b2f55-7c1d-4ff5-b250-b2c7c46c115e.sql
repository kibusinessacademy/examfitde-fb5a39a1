...
-- Fix 1: Promote tier1_passed → approved for Kfz-Mechatroniker
UPDATE public.exam_questions
SET qc_status = 'approved'
WHERE curriculum_id = 'fbc805ce-e798-4cf6-a189-20f147ae0f38'
  AND qc_status = 'tier1_passed'
  AND status = 'approved';

-- Fix 2: Update RPC to count tier1_passed as coverage-equivalent
...
BEGIN
  ...
  -- LF coverage: count approved AND tier1_passed as covered
  ...
  WHERE curriculum_id = p_curriculum_id
    AND (
      qc_status IN ('approved', 'tier1_passed')
      OR (qc_status IS NULL AND status = 'approved')
    )
    AND learning_field_id IS NOT NULL;

  -- Competency coverage: also count tier1_passed
  ...
  -- Missing trap metadata (keep strict: only fully approved)
  ...
  -- Missing bloom metadata (keep strict: only fully approved)
