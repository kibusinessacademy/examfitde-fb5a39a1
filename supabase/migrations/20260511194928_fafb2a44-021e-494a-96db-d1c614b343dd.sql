...
DECLARE
  ...
  v_count int; v_skip_audit_present boolean; v_success_audit_count int;
  v_status_after text;
BEGIN
  ...
  -- T3 (v2 path): blocked tail step >5min alt → status flip + success audit.
  -- Hinweis: fn_detect_tail_step_enqueue_drift (v1) skipt eigene Pakete via
  -- fn_is_package_progress_blocked (preexisting Guard); deshalb prüfen wir
  -- hier den v2-Healer, der den Status-Flip vornimmt.
  ...
  VALUES (gen_random_uuid(), v_pkg_id, 'auto_publish', 'blocked'::step_status,
          now() - interval '3 hours', '{}'::jsonb);
  PERFORM public.fn_detect_and_heal_tail_step_enqueue_drift_v2();
  SELECT status::text INTO v_status_after FROM package_steps
    WHERE package_id=v_pkg_id AND step_key='auto_publish';
  SELECT count(*) INTO v_success_audit_count FROM auto_heal_log
    WHERE action_type='tail_step_drift_v2_heal'
      AND result_status='success'
      AND target_id = v_pkg_id::text
      AND created_at > now() - interval '1 minute';
  v_results := v_results || jsonb_build_object(
    'test','T3_blocked_promoted_via_v2',
    'pass', v_status_after='queued' AND v_success_audit_count=1,
    'status_after', v_status_after, 'success_audits', v_success_audit_count);
  IF v_status_after='queued' AND v_success_audit_count=1
    THEN v_passed:=v_passed+1; ELSE v_failures:=v_failures+1; END IF;

  -- T4: zweiter Lauf <5min muss skipped werden (cooldown), kein zweiter
  -- success-Audit, dafür tail_heal_skipped_package_cooldown.
  -- Step erneut blocked machen, ohne success-Audit zu löschen.
  ...
  v_results := v_results || jsonb_build_object(
    ...
    'pass', v_status_after='blocked' AND v_skip_audit_present,
    'status_after', v_status_after, 'skip_audit', v_skip_audit_present);
  IF v_status_after='blocked' AND v_skip_audit_present
    THEN v_passed:=v_passed+1; ELSE v_failures:=v_failures+1; END IF;
  ...
  DELETE FROM auto_heal_log WHERE target_id = v_pkg_id::text;
