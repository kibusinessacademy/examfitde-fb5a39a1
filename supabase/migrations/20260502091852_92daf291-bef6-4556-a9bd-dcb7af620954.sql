...
BEGIN
  ...
  FOR v_rec IN
    SELECT DISTINCT ps.package_id, cp.curriculum_id, ps.step_key, 'package_' || ps.step_key AS job_type
    FROM package_steps ps
    JOIN course_packages cp ON cp.id = ps.package_id
  ...
  LOOP
    BEGIN
      INSERT INTO job_queue(
        job_type, status, payload, priority, max_attempts, lane, worker_pool, package_id, run_after, created_at, updated_at, meta
      )
      VALUES (
        ...
        v_rec.package_id,
        now(), now(), now(),
        jsonb_build_object('enqueue_source','pattern_x13_drift_heal','pattern','X13')
      );
