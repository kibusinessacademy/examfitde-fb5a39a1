...
pkg_batches AS (
  ...
  FROM job_queue jq
  JOIN llm_batch_requests lbr
    ON lbr.source_job_id = jq.id
  JOIN llm_batches lb
    ON lb.id = lbr.batch_id
...
SELECT
  ...
  CASE
    -- RETRYING before HEALTHY_ACTIVE (otherwise unreachable)
    ...
    -- POISONED_LOOP before SHADOW_ZOMBIE (stronger diagnosis first)
    WHEN COALESCE(pj.active_jobs, 0) > 0
      AND COALESCE(pj.completed_1h, 0) = 0
