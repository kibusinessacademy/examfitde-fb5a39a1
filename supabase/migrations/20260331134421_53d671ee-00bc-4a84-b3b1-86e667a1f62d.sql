-- Fix recover_and_reenter_package: replace non-existent 'completed_at' with 'finished_at'
...
BEGIN
  ...
  -- Reset failed/blocked/timeout steps to queued (use finished_at, not completed_at)
  WITH reset AS (
  ...
  IF v_eligible THEN
    ...
    UPDATE public.course_packages
    SET status = 'building', stuck_reason = null, updated_at = v_now
  ...
  VALUES ('recover_and_reenter_package', p_trigger_source, 'course_package', p_package_id::text,
    ...
    jsonb_build_object('package_status_before', v_pkg.status, 'final_status', v_final_status, 'reset_steps', v_reset_steps, 'reason', p_reason, 'actor_user_id', p_actor_user_id));
