...
BEGIN
  ...
  LOOP
    ...
    VALUES (v_step.package_id, v_job_type, 'pending', 5, jsonb_build_object('source', 'auto_heal_stale_lock'), now());
...
BEGIN
  ...
  LOOP
    ...
    IF v_q_count >= 50 THEN
      ...
      INSERT INTO job_queue (package_id, job_type, status, priority, payload, created_at)
      VALUES (v_step.package_id, 'package_validate_exam_pool', 'pending', 5, jsonb_build_object('source', 'auto_heal_repair_exhausted', 'q_count', v_q_count), now());
  ...
  LOOP
    IF v_step.step_key = 'generate_lesson_minichecks' THEN
      ...
      IF v_artifact_count >= 5 THEN
        ...
        INSERT INTO job_queue (package_id, job_type, status, priority, payload, created_at)
        VALUES (v_step.package_id, 'package_generate_lesson_minichecks', 'pending', 5, jsonb_build_object('source', 'auto_heal_materialization', 'artifacts', v_artifact_count), now());
