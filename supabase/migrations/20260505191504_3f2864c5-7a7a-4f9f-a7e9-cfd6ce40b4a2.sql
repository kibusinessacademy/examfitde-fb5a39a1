...
BEGIN
  ...
  IF p_email !~* '@examfit-smoke\.local$' THEN
    RAISE EXCEPTION 'only @examfit-smoke.local emails are allowed';
  END IF;
  ...
  INSERT INTO public.admin_actions (user_id, action, payload)
  VALUES (v_caller, 'smoke_password_reset',
          jsonb_build_object('email', p_email, 'user_id', v_user_id, 'caller_role', v_role, 'session_user', v_session_user));
