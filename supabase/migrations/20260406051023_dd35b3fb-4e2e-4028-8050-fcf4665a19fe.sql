...
BEGIN
  FOR pkg IN
    SELECT cp.id, cp.title AS pkg_title, cp.build_progress, cp.status, cp.track,
  ...
  LOOP
    ...
    LOOP
      ...
      IF rule.rule_type = 'track' AND pkg.track = rule.pattern THEN
        CASE rule.dimension
          WHEN 'reuse' THEN s_reuse := GREATEST(s_reuse, rule.weight);
          WHEN 'monetization' THEN s_monet := GREATEST(s_monet, rule.weight);
          ELSE NULL;
    ...
    -- Pipeline: build_progress = 0 → highest kickstart value
    ...
    -- Weighted: demand 30%, monet 30%, pipeline 10%, reuse 20%, freshness 10%
