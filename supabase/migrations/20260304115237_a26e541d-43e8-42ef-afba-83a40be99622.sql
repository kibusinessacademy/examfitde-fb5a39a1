...
-- Fix gap report RPC: competencies → learning_fields → curriculum_id
...
BEGIN
  ...
  -- Competencies with < 3 approved questions (join via learning_fields)
  ...
  FROM (
    SELECT c.id as comp_id,
           (SELECT count(*) FROM exam_questions eq WHERE eq.competency_id = c.id AND eq.status = 'approved') as approved_count
    FROM competencies c
