...
-- Content-Gap candidates view
CREATE OR REPLACE VIEW public.v_content_gap_candidates AS
SELECT
  cp.id AS package_id,
  cp.package_key,
  cp.title,
  ...
  cp.status,
  cp.blocked_reason,
  COALESCE(aq.approved_count, 0)::int AS approved_count,
  GREATEST(50 - COALESCE(aq.approved_count, 0), 0)::int AS shortfall_to_min,
  (cp.feature_flags->'content_gap_topup'->>'last_dispatched_at')::timestamptz AS last_dispatched_at,
  COALESCE((cp.feature_flags->'content_gap_topup'->>'attempts')::int, 0) AS topup_attempts
FROM public.course_packages cp
LEFT JOIN LATERAL (
  SELECT count(*)::int AS approved_count
  FROM public.exam_questions eq
  WHERE eq.curriculum_id = cp.curriculum_id AND eq.status = 'approved'
) aq ON true
WHERE cp.blocked_reason = 'auto_heal_zombie'
  AND COALESCE(aq.approved_count, 0) < 50;

REVOKE ALL ON public.v_content_gap_candidates FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.v_content_gap_candidates TO service_role;

-- Admin-gated read RPC
CREATE OR REPLACE FUNCTION public.admin_get_content_gap_candidates()
RETURNS SETOF public.v_content_gap_candidates
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  RETURN QUERY SELECT * FROM public.v_content_gap_candidates ORDER BY shortfall_to_min ASC;
END;
$$;

REVOKE ALL ON FUNCTION public.admin_get_content_gap_candidates() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_get_content_gap_candidates() TO authenticated, service_role;

-- Targeted top-up dispatch (no status bypass, no force-publish, dedup-safe)
...
BEGIN
  ...
  -- Dedup: kein zweiter aktiver Repair pro Paket
  SELECT count(*)::int INTO v_active_jobs
  FROM job_queue
  WHERE package_id = p_package_id
    AND status IN ('queued','processing','pending_enqueue','building')
...
REVOKE ALL ON FUNCTION public.admin_content_gap_topup_dispatch(uuid, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_content_gap_topup_dispatch(uuid, boolean) TO authenticated, service_role;

-- Audit recheck: unblock if approved >= 50 and hand back to council
...
REVOKE ALL ON FUNCTION public.admin_content_gap_audit_recheck(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_content_gap_audit_recheck(uuid) TO authenticated, service_role;
