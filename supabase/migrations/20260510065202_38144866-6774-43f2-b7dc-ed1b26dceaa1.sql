...
BEGIN
  ...
  -- Governance steps are reconciled exclusively by fn_trg_job_complete_reconcile_step.
  -- The generic sync must not mark them done blindly, otherwise failed gates block the
  -- job_queue completion update via governance finalization guard and leave jobs processing.
  IF NEW.job_type IN ('package_run_integrity_check','package_quality_council','package_auto_publish') THEN
    RETURN NEW;
  END IF;
  ...
  IF v_step_key = ANY(v_lc_steps) THEN
    ...
    JOIN public.learning_fields lf ON lf.curriculum_id = cp.curriculum_id
    JOIN public.competencies co ON co.learning_field_id = lf.id
    JOIN public.lessons l ON l.competency_id = co.id
    ...
    IF v_is_hollow AND v_total_lessons > 0 THEN
      ...
      SET status = 'queued'::public.step_status,
          last_error = format('B3 Hollow-Guard: %s/%s real, %s placeholders, ratio=%.2f', v_real_lessons, v_total_lessons, v_placeholder_lessons, v_substantive_ratio),
  ...
  SET status = 'done'::public.step_status,
      started_at = COALESCE(started_at, NEW.started_at, NEW.locked_at, now() - interval '1 second'),
...
INSERT INTO public.auto_heal_log(action_type, trigger_source, target_type, target_id, result_status, result_detail, metadata)
VALUES ('job_runner_governance_sync_fix','migration','system','job_runner','success',
        'Generic job-complete step sync no longer finalizes governance steps blindly.',
        jsonb_build_object('affected_trigger','fn_trigger_sync_step_on_job_complete','governance_owner','fn_trg_job_complete_reconcile_step'));