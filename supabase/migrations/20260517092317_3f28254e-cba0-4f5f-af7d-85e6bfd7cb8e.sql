    ...
    BEGIN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'curriculum_id', (SELECT cp.curriculum_id FROM public.course_packages cp WHERE cp.id = r.package_id)
        ),
        'pending',
        5,
...
-- Wave 2 live dispatch
DO $$
DECLARE
  v_result jsonb;
BEGIN
  v_result := public.admin_dispatch_lessons_gap_repair(
    _wave_size := 10,
    _wip_cap := 20,
    _force_no_modules := false,
    _dry_run := false
  );
  ...
  VALUES (
    ...
    'system',
    'ok',
    jsonb_build_object('wave', 2, 'dispatch_result', v_result)
  );

  RAISE NOTICE 'Wave 2 result: %', v_result;
END $$;