...
DECLARE
  ...
  v_reason text := 'E3f publish-gate wave3 promote remaining ready pillars to live';
BEGIN
  FOR v_rec IN
    ...
    BEGIN
      ...
      ELSE
        ...
        PERFORM public.fn_emit_audit(
          ...
          jsonb_build_object('pillar_id', v_rec.pillar_id,'skip_reason','ALREADY_PUBLISHED_OR_NOOP','wave','wave3'),
          'admin_publish_certification_pillars');
      END IF;
    EXCEPTION WHEN OTHERS THEN
      ...
      BEGIN
        PERFORM public.fn_emit_audit(
          ...
          jsonb_build_object('pillar_id', v_rec.pillar_id,'skip_reason','EXCEPTION','error', SQLERRM,'wave','wave3'),
  ...
  RAISE NOTICE 'E3f Wave3: attempted=% published=% skipped=% failed=%',
