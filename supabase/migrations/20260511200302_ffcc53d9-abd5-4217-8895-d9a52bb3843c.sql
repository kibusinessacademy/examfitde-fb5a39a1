DROP FUNCTION IF EXISTS public.fn_tail_heal_package_cooldown_active(uuid, interval);

CREATE FUNCTION public.fn_tail_heal_package_cooldown_active(
  _package_id uuid,
  _window interval DEFAULT interval '5 minutes'
)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $$
  SELECT EXISTS (
    ...
    WHERE l.action_type IN (
      ...
      AND l.result_status IN ('success','skipped')
      AND (l.metadata->>'package_id')::uuid = _package_id
      AND l.created_at > now() - _window
  );
$$;
...
GRANT EXECUTE ON FUNCTION public.admin_smoke_tail_healer_coordination_v1() TO authenticated, service_role;