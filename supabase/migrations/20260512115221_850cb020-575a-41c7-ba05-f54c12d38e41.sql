...
DECLARE
  ...
  v_allowed bool := (
    ...
    OR (v_uid IS NOT NULL AND public.has_role(v_uid,'admin'::app_role))
  );
  v_i int := 0;
  v_n int;
  v_snap jsonb;
  v_clean_count int;
  v_stop text := NULL;
  v_failure_classes text[] := ARRAY['POOL_GAP_REPAIR','BLOOM_GAP_REPAIR','TRAP_GAP_REPAIR'];
  v_new_failures int;
  v_baseline_failures int;
BEGIN
  IF NOT v_allowed THEN
    RAISE EXCEPTION 'forbidden: admin or service_role required (role=%, user=%)', v_role, current_user;
  END IF;
  ...
  WHILE v_i < p_max_batches LOOP
    ...
    FROM (SELECT gate_class, count(*) AS n FROM public.v_publish_readiness_gate GROUP BY gate_class) s;
    ...
    VALUES (
      ...
      jsonb_build_object('batch_no',v_i,'enqueued_count',v_n,'gate_snapshot',v_snap,
        'bronze_clean_remaining',v_clean_count - v_n,'failure_classes_count',v_new_failures,
        'baseline_failures',v_baseline_failures)
    );

    batch_no := v_i; enqueued_count := v_n; gate_snapshot := v_snap; stopped_reason := NULL;
    RETURN NEXT;

    IF v_n = 0 THEN v_stop := 'empty_batch'; EXIT; END IF;
  ...
  batch_no := -1; enqueued_count := 0; gate_snapshot := v_snap;
  stopped_reason := COALESCE(v_stop, 'max_batches_reached');
  RETURN NEXT;
END;
$$;

GRANT EXECUTE ON FUNCTION public.admin_drain_queued_tail_auto(int,int,int,int,bool) TO authenticated, service_role;