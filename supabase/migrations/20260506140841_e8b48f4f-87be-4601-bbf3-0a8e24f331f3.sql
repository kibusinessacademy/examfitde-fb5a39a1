...
-- Drop fehlerhaften Index aus vorigem Versuch
DROP INDEX IF EXISTS public.entitlements_user_curriculum_type_uniq;
  ...
  LOOP
    IF NOT p_dry_run THEN
      INSERT INTO public.entitlements(
        user_id, curriculum_id, product_id,
        valid_from, valid_until, source, source_type,
        has_learning_course, has_exam_trainer, has_ai_tutor, has_oral_trainer
      )
      VALUES (
        r.user_id, r.curriculum_id, r.product_id,
        now(), now()+interval '12 months', 'web', 'web',
        true, true, true, true
      );
