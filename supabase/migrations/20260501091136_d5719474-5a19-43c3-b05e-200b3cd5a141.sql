...
-- Erlaube service_role-Bypass (für CI/Backoffice ohne JWT)
...
BEGIN
  ...
  IF p_apply THEN
    WITH ranked AS (
      SELECT p.id, p.certification_id AS cert_id, p.title,
        EXISTS(SELECT 1 FROM course_packages cp WHERE cp.product_id=p.id AND cp.status='published') AS pub_ref,
        EXISTS(SELECT 1 FROM product_prices pr WHERE pr.product_id=p.id AND pr.active=true) AS has_price,
        ROW_NUMBER() OVER (
          PARTITION BY p.certification_id
    ...
    clusters AS (SELECT cert_id FROM ranked GROUP BY cert_id HAVING COUNT(*) > 1),
    to_archive AS (
      SELECT r.id, r.cert_id, r.title FROM ranked r JOIN clusters c ON c.cert_id=r.cert_id
    ...
    audit_ins AS (
      ...
      SELECT NULL, 'archive_duplicate_product', NULL, NULL, NULL, NULL,
