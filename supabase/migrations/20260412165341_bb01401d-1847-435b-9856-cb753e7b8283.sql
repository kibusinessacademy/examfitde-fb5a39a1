...
-- Case 1a: integrity_passed=true, meta.ok missing, but package is legacy (no meta.executed)
SELECT 'legacy_pre_meta_ok'::text AS drift_type,
...
WHERE cp.integrity_passed = true
  ...
  AND (ps.meta IS NULL OR ps.meta = '{}'::jsonb OR ps.meta->>'executed' IS NULL)

UNION ALL

-- Case 1b: integrity_passed=true, meta.ok not true, post-fix package (has meta.executed)
SELECT 'post_fix_integrity_without_meta_ok',
...
WHERE cp.integrity_passed = true
  AND (ps.meta->>'ok')::text IS DISTINCT FROM 'true'
  AND ps.meta IS NOT NULL AND ps.meta != '{}'::jsonb
  AND ps.meta->>'executed' IS NOT NULL

UNION ALL

-- Case 2: meta.ok=true but integrity_passed != true (post-fix only)
