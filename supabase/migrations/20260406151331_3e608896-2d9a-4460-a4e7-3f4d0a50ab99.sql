...
DECLARE
  ...
  -- Fan-out job types: parent enqueues sub-jobs with discriminator keys
  -- For these, we also check for active sub-jobs before re-creating the parent
...
BEGIN
  FOR rec IN
    ...
    WHERE ps.status = 'queued'
      ...
      -- ═══ IDEMPOTENCY GUARD: Inflight + 10min Failure Cooldown ═══
      -- Block 1: No active job (pending / processing / batch_pending)
      AND NOT EXISTS (
      ...
      -- Block 2: No recently failed job (10-min cooldown)
      AND NOT EXISTS (
        SELECT 1 FROM job_queue jq
        WHERE jq.package_id = ps.package_id
          AND jq.job_type = (v_step_jobs ->> ps.step_key)
      ...
      -- ═══ FAN-OUT GUARD ═══
      -- Block 3: For fan-out steps, don't re-create parent if sub-jobs are still active
      AND NOT (
        (v_step_jobs ->> ps.step_key) = ANY(v_fanout_job_types)
        AND EXISTS (
          SELECT 1 FROM job_queue jq
          WHERE jq.package_id = ps.package_id
            AND jq.job_type = (v_step_jobs ->> ps.step_key)
            AND jq.status IN ('pending', 'processing', 'batch_pending')
            -- Sub-jobs have a discriminator key (blueprintId, lesson_id, etc.)
            AND (
