...
-- Fix acquire_next_package_lease_v2: orphan reclaim should not depend on updated_at
-- The updated_at column is constantly refreshed by triggers, preventing reclaim.
-- New approach: check if the package has been building without a lease for > 30 seconds
-- using the absence of any lease record (not just expired ones) as the staleness signal.
...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════
  -- ORPHAN RECLAIM: Reset 'building' packages that have no active
  -- lease AND no active jobs back to 'queued'.
  -- FIX: No longer depends on updated_at which gets refreshed by
  -- triggers. Instead uses a short 30s window based on when the
  -- lease was last deleted (no lease record exists at all).
  -- ══════════════════════════════════════════════════════════════
  ...
  IF v_orphan_reclaimed > 0 THEN
    RAISE LOG '[acquire_v2] Reclaimed % orphan building packages (no lease, no active jobs, no active steps)', v_orphan_reclaimed;
  END IF;
