...
SELECT
  ...
  percentile_cont(0.50) WITHIN GROUP (ORDER BY queue_wait_ms)
    FILTER (WHERE status='completed' AND queue_wait_ms IS NOT NULL) AS qwait_p50_ms,
  percentile_cont(0.95) WITHIN GROUP (ORDER BY queue_wait_ms)
    FILTER (WHERE status='completed' AND queue_wait_ms IS NOT NULL) AS qwait_p95_ms,
  percentile_cont(0.50) WITHIN GROUP (ORDER BY run_ms)
    FILTER (WHERE status='completed' AND run_ms IS NOT NULL) AS run_p50_ms,
  percentile_cont(0.95) WITHIN GROUP (ORDER BY run_ms)
    FILTER (WHERE status='completed' AND run_ms IS NOT NULL) AS run_p95_ms,
