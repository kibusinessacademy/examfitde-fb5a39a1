...
BEGIN
  ...
  FROM public.course_packages WHERE curriculum_id IS NOT NULL LIMIT 1;
  IF v_curriculum_id IS NULL THEN
    RAISE EXCEPTION 'no curriculum_id available for smoke';
  END IF;

  v_pkg_id := gen_random_uuid();
  -- 'archived' status sits outside uniq_visible_package_per_curriculum partial index
  INSERT INTO public.course_packages(id, package_key, title, status, curriculum_id, created_at, updated_at)
  VALUES (v_pkg_id, 'smoke_tail_heal_'||substr(v_pkg_id::text,1,8), 'smoke', 'archived', v_curriculum_id, now(), now());

  -- For tail-healer logic we need building. Trigger may complain on status change — try direct UPDATE bypass via session_replication_role
  PERFORM set_config('session_replication_role', 'replica', true);
  UPDATE public.course_packages SET status='building' WHERE id = v_pkg_id;
