...
BEGIN
  ...
  -- Index-friendly: filter created_at first; only then category-filter
  WITH evt AS (
    SELECT
      id, created_at, action_type, result_status, trigger_source,
      target_id, target_type, error_message, duration_ms, metadata,
      CASE
        WHEN action_type ILIKE 'verwaltung%' THEN 'verwaltung_native'
        WHEN action_type ILIKE '%tutor%' THEN 'tutor_governance'
        WHEN action_type ILIKE '%refusal%' THEN 'refusal_event'
        ELSE 'general'
      END AS audit_category
    FROM public.auto_heal_log
    WHERE created_at > now() - make_interval(days => _window_days)
      AND (
        action_type ILIKE 'verwaltung%'
        OR target_type IN ('verwaltung_oral_session','verwaltung_workflow','verwaltung_agent')
        OR (metadata ? 'department_key')
      )
  ),
  ...
  FROM (
    SELECT
      ...
      CASE
        ...
        ELSE 'general'
      END AS audit_category,
      result_status, trigger_source, target_id, target_type,
  ...
  PERFORM public.fn_emit_audit(
    _action_type := 'verwaltung_governance_audit_trail_read',
    _payload := jsonb_build_object('window_days', _window_days, 'caller_role', v_role)
  );
...
BEGIN
  ...
  PERFORM public.fn_emit_audit(
    _action_type := 'verwaltung_governance_refusal_quality_read',
    _payload := jsonb_build_object('window_days', _window_days, 'caller_role', v_role)
  );
  RETURN v_payload;
END; $$;
...
BEGIN
  ...
  PERFORM public.fn_emit_audit(
    _action_type := 'verwaltung_governance_source_coverage_read',
    _payload := jsonb_build_object('window_days', _window_days, 'caller_role', v_role)
  );
  RETURN v_payload;
END; $$;
