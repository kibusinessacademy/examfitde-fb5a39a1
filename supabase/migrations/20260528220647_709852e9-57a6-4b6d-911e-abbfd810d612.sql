...
BEGIN
  ...
  SELECT id, user_id,
         (scores  IS NOT NULL AND scores  <> '{}'::jsonb
          AND debrief IS NOT NULL AND debrief <> '{}'::jsonb)
  ...
  PERFORM public.fn_emit_audit(
    _action_type := 'verwaltung_realtime_debrief_generated',
    _payload := jsonb_build_object(
      'convai_session_id', _convai_session_id,
      'session_id',        v_session_id,
      'user_id',           v_user_id,
      'overall_score',     COALESCE((_scores->>'overall')::int, 0),
      'caller_role',       'service_role'
    )
  );
