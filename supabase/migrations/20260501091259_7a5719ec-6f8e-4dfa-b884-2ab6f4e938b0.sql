...
BEGIN
  ...
  -- Apply ist gesperrt für read-only sessions
  IF p_apply AND session_user = 'supabase_read_only_user' THEN
    RAISE EXCEPTION 'apply not allowed in read-only session';
  END IF;

  RETURN QUERY
  WITH ranked AS (
    SELECT p.id, p.certification_id AS cert_id, p.title, p.updated_at,
...
GRANT EXECUTE ON FUNCTION public.admin_merge_duplicate_certification_products(boolean) TO service_role, supabase_read_only_user;
