...
BEGIN
  -- Get curriculum_id for payload guard
  ...
  ins AS (
    INSERT INTO public.job_queue (
      package_id, job_type, status, payload, created_at, updated_at, curriculum_id
    )
    SELECT
      p_package_id, 'lesson_regen_repair', 'pending',
      jsonb_build_object('lesson_id', c.lesson_id, 'reason', 'needs_regen_backfill', 'source', 'enqueue_learning_content_regen_for_package', 'curriculum_id', v_curriculum_id),
      now(), now(), v_curriculum_id
