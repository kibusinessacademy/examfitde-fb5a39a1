...
-- 1. Make porosity configurable from ops_pipeline_config
...
-- 2. Scheduler Fairness Dashboard View
CREATE OR REPLACE VIEW v_scheduler_fairness AS
SELECT
  cp.id,
  left(cp.title, 50) as title,
  cp.status,
  cp.priority as base_priority,
  cp.ready_since,
  round(extract(epoch from (now() - cp.ready_since)) / 60) as wait_minutes,
  LEAST(3, COALESCE(floor(extract(epoch from (now() - cp.ready_since)) / 5400)::int, 0)) as aging_bonus,
  GREATEST(3, COALESCE(cp.priority, 999) - LEAST(3, COALESCE(floor(extract(epoch from (now() - cp.ready_since)) / 5400)::int, 0))) as effective_priority,
  CASE
    WHEN COALESCE(cp.priority, 999) <= 50 THEN 'within_gate'
    WHEN cp.ready_since < now() - interval '6 hours' THEN 'aging_override'
    ELSE 'behind_gate'
  END as gate_status,
  CASE
    WHEN cp.status = 'building' AND NOT EXISTS (
      SELECT 1 FROM package_leases pl WHERE pl.package_id = cp.id AND pl.lease_until > now()
    ) THEN 'resumable'
    WHEN cp.status = 'building' THEN 'active_lease'
    ELSE cp.status
  END as claim_status,
...
WHERE cp.status IN ('queued', 'building', 'failed')
ORDER BY
  CASE WHEN cp.status = 'building' THEN 0 WHEN cp.status = 'queued' THEN 1 ELSE 2 END,
  GREATEST(3, COALESCE(cp.priority, 999) - LEAST(3, COALESCE(floor(extract(epoch from (now() - cp.ready_since)) / 5400)::int, 0))) ASC,
  cp.ready_since ASC NULLS LAST;
