-- ============================================================
-- Heal RPC Hotfix v3 — Council-Resume curriculum_id SSOT-Compliance
-- Bug: assert_job_payload erfordert curriculum_id, aber beide
-- Resume-RPCs übergaben nur package_id → SSOT VIOLATION
-- ============================================================

-- 1) Single Resume — curriculum_id aus course_packages laden
...
BEGIN
  ...
  -- SSOT: curriculum_id aus course_packages
  ...
  -- Job mit SSOT-konformem Payload (curriculum_id Pflicht!)
...
-- 2) Bulk Resume — curriculum_id pro Paket laden, Pakete ohne Curriculum skippen
...
BEGIN
  ...
  FOREACH v_pkg IN ARRAY v_targets LOOP
    SELECT cp.curriculum_id INTO v_curr
    FROM public.course_packages cp
    WHERE cp.id = v_pkg;

    IF v_curr IS NULL THEN
      v_per_pkg := v_per_pkg || jsonb_build_object(
        'package_id', v_pkg, 'action', 'skipped_no_curriculum'
      );
    ...
    v_per_pkg := v_per_pkg || jsonb_build_object(
      'package_id', v_pkg, 'curriculum_id', v_curr, 'action', 'resumed'
    );
    v_total := v_total + 1;
  END LOOP;
  ...
  VALUES (
    ...
    jsonb_build_object(
      'packages_resumed', v_total,
      'packages_skipped_no_curriculum', v_skipped
    ),
    'package',
    v_targets
  );
  ...
  RETURN jsonb_build_object(
    ...
    'dry_run', false,
    'packages', v_total,
    'skipped_no_curriculum', v_skipped,
