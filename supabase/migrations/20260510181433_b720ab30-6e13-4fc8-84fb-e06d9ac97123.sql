...
DECLARE
  ...
  v_artifact_jobs     text[] := ARRAY[
    'package_post_publish_blog','seo_indexnow_submit','seo_sitemap_refresh','seo_internal_links',
...
BEGIN
    ...
    LOOP
      ...
      BEGIN
        INSERT INTO public.job_queue (
          job_type, package_id, status, idempotency_key, payload, meta, created_at
        ) VALUES (
        ...
        VALUES ('post_publish_growth_repair:'||r.job_type, r.package_id::text, 'package',
                ...
                jsonb_build_object('job_type', r.job_type, 'idempotency_key', v_idem,
                                   'curriculum_id', r.curriculum_id, 'job_id', v_inserted_id));
      ...
      WHEN OTHERS THEN
        ...
        VALUES ('post_publish_growth_repair:'||r.job_type, r.package_id::text, 'package',
                'failed', SQLERRM, jsonb_build_object('job_type', r.job_type));
      END;
    END LOOP;
  ...
  IF p_repair OR v_drift_total > 0 THEN
    INSERT INTO public.auto_heal_log(action_type, target_id, target_type, result_status, result_detail, metadata)
    VALUES (
      'post_publish_growth_health_check', 'system','system',
