...
BEGIN
  ...
  -- Find the minimum priority among all incomplete (non-terminal) packages.
  -- Only packages with this priority (or rebuilds) may be acquired.
  -- This ensures Prio 1 finishes before Prio 2, Prio 2 before Prio 3, etc.
  -- ══════════════════════════════════════════════════════════════
  ...
  -- If nothing incomplete, nothing to acquire
  IF v_min_incomplete_priority IS NULL THEN RETURN NULL; END IF;
  ...
  LOOP
    ...
    WHERE
      ...
      -- STRICT TIER GATE: only allow packages at the lowest incomplete priority
      -- Rebuilds bypass this gate
      AND (
        cp.is_rebuild = true
        OR COALESCE(cp.priority, 999999) <= v_min_incomplete_priority
      )
    ORDER BY
      -- Rebuilds get top priority
