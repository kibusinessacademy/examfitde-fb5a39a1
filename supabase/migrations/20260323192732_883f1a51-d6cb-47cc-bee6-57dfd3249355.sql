...
-- Harden fn_is_true_stall: add special job-type variants for critical steps
...
BEGIN
  ...
  SELECT bool_and(ps2.status IN ('done', 'skipped'))
    ...
    FROM pipeline_step_edges e
    JOIN package_steps ps2
      ON ps2.package_id = p_package_id AND ps2.step_key = e.from_step
  ...
  SELECT count(*)
   ...
   WHERE jq.package_id = p_package_id
     ...
     AND (
       ...
       -- Special variants for critical steps
