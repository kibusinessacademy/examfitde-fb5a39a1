-- Heal-Cockpit v8.4.1 — Reason-Marker-Hygiene
-- Refactor v_admin_heal_cockpit so dass recommended_action in einer CTE
-- vorab berechnet wird und der Reason-Marker `awaiting_pipeline_start=true`
-- nur noch dann gesetzt wird, wenn die finale Action tatsächlich
-- 'awaiting_pipeline' ist. Verhindert widersprüchliche Reason-Marker
-- bei höher priorisierten Actions (z.B. guided_recovery).
...
SELECT package_id,
       ...
       array_remove(ARRAY[
         ...
         -- Reason-Marker nur wenn finale Action awaiting_pipeline ist
