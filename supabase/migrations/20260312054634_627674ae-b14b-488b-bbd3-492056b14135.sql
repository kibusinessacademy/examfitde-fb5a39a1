...
BEGIN
  ...
  -- lease back to 'queued'. These are orphans from crashed runners
  -- that would otherwise permanently block WIP slots.
  -- Only reclaim packages that have been building for > 3 minutes
  -- (to avoid reclaiming packages mid-transition).
  -- ══════════════════════════════════════════════════════════════
  UPDATE public.course_packages cp
  SET status = 'queued',
      updated_at = now()
  WHERE cp.status = 'building'
    AND cp.updated_at < now() - interval '3 minutes'
  ...
  IF v_orphan_reclaimed > 0 THEN
    RAISE LOG '[acquire_v2] Reclaimed % orphan building packages (no lease, no active jobs)', v_orphan_reclaimed;
  END IF;
