-- Fix 1: DAG-aware cascade reset (replaces broken linear cascade)
-- The old trigger used linear step order, cascading resets across independent parallel branches.
-- This version uses the actual DAG dependency graph.
...
DECLARE
  ...
  -- A step is "downstream" of the reset step if it transitively depends on it
  v_dag jsonb := '{
    ...
    "build_ai_tutor_index": ["validate_learning_content"],
    "validate_tutor_index": ["build_ai_tutor_index"],
    "generate_oral_exam": ["validate_learning_content"],
    ...
    "elite_harden": ["validate_learning_content"],
...
-- Fix 2: Clear tier1_failed qc_status when content is regenerated
-- Add a trigger that resets qc_status when a new approved content_version is inserted
-- for a lesson that was tier1_failed
CREATE OR REPLACE FUNCTION clear_tier1_failed_on_new_content()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  IF NEW.status = 'approved' AND NEW.lesson_id IS NOT NULL THEN
    -- Only act on tier1_failed lessons to avoid interfering with normal flow
    UPDATE lessons
...
DROP TRIGGER IF EXISTS trg_clear_tier1_failed_on_new_content ON content_versions;
CREATE TRIGGER trg_clear_tier1_failed_on_new_content
  AFTER INSERT ON content_versions
  FOR EACH ROW
  WHEN (NEW.status = 'approved')
  EXECUTE FUNCTION clear_tier1_failed_on_new_content();