...
-- 1. Replace guard with proper bypass support
CREATE OR REPLACE FUNCTION public.guard_learning_content_done()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  v_bypass text;
BEGIN
  IF NEW.step_key <> 'generate_learning_content' OR NEW.status <> 'done' THEN
    RETURN NEW;
  END IF;

  v_bypass := current_setting('app.reconcile_bypass', true);
  IF v_bypass = 'on' THEN
    RETURN NEW;
  END IF;
  ...
  JOIN public.course_packages cp ON cp.course_id = m.course_id
  ...
  IF v_broken_count > 0 THEN
    RAISE EXCEPTION
      'GUARD_LEARNING_CONTENT_DONE: Cannot mark step done — % broken lessons remain (content NULL/placeholder/tier1_failed/regenerating)',
      v_broken_count;
  END IF;
...
-- 2. Replace reconcile function with strict broken_lessons = 0 rule
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE COALESCE(l.step, '') <> 'mini_check'
          AND (
            l.content IS NULL
            OR l.content->>'_placeholder' = 'true'
            OR l.qc_status = 'tier1_failed'
