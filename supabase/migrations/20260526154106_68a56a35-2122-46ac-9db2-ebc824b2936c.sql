...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      IF v_is_override THEN
        PERFORM fn_emit_audit(
          _action_type := 'queued_tail_reconciler_v2_override',
          _target_type := 'package',
          _target_id   := rec.package_id::text,
          _result_status := 'success',
          _payload := jsonb_build_object(
    ...
    EXCEPTION WHEN OTHERS THEN
      v_skipped := v_skipped + 1;
      PERFORM fn_emit_audit(
        _action_type := 'queued_tail_reconciler_v2_error',
        _target_type := 'package',
        _target_id   := rec.package_id::text,
  ...
  PERFORM fn_emit_audit(
    _action_type := 'queued_tail_reconciler_v2_run_summary',
    _target_type := 'system',
    _result_status := CASE WHEN p_dry_run THEN 'dry_run' ELSE 'success' END,
    _payload := jsonb_build_object(
