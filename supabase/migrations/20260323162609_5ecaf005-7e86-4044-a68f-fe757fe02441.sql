...
DROP FUNCTION IF EXISTS public.heal_learning_content_deadlock(uuid, numeric, boolean);

CREATE FUNCTION public.heal_learning_content_deadlock(
...
RETURNS TABLE (
  out_package_id uuid,
  out_package_title text,
  out_total_lessons integer,
  out_generated_lessons integer,
  out_completion_ratio numeric,
  out_needs_regen_count integer,
...
BEGIN
  ...
  LOOP
    ...
    IF v_total > 0 AND v_ratio >= p_completion_threshold THEN
      ...
      SET status = 'done', finished_at = COALESCE(ps.finished_at, now()), updated_at = now(), last_error = NULL,
      ...
      WHERE ps.package_id = r.id AND ps.step_key IN ('finalize_learning_content', 'validate_learning_content') AND ps.status IN ('queued', 'enqueued');
