...
-- =============================================================================
-- admin_reconcile_bronze_no_report — Bucket C Heal-RPC (α)
-- Lehre 2026-05-13: 21 BRONZE_REVIEW_REQUIRED Pakete haben score=0 + hard_fails=[]
-- weil nach Bronze-Lock nie wieder run_integrity_check lief.
-- Strukturheilung: Report-Rebuild (NICHT Unlock, NICHT Content-Mutate).
-- =============================================================================
...
BEGIN
  -- Admin-Gate (NULL caller = service-role / SQL-Console erlaubt)
  ...
  FOR rec IN
    SELECT prg.package_id, cp.package_key, cp.curriculum_id, prg.score, prg.hard_fail_reasons
    ...
    LIMIT GREATEST(p_limit, 0) * 4 + 10  -- Headroom für Filter-Skips
  LOOP
    -- (1) approved-Pool prüfen (>= 50)
    ...
    IF v_active > 0 THEN
      ...
      VALUES (rec.package_id::text, 'package',
              ...
              jsonb_build_object(
                'package_id', rec.package_id,
                'package_key', rec.package_key,
                'reason', 'active_integrity_job_present',
    ...
    IF p_dry_run THEN
      ...
      VALUES (rec.package_id::text, 'package',
              ...
              jsonb_build_object(
                ...
                'run_id', v_run_id,
                'would_enqueue', 'package_run_integrity_check',
      ...
      BEGIN
        ...
        VALUES (rec.package_id::text, 'package',
                ...
                jsonb_build_object(
                  ...
                  'curriculum_id', rec.curriculum_id,
                  'run_id', v_run_id,
                  'enqueued', 'package_run_integrity_check',
                  'bronze_lock_override', true));
    ...
    -- Limit nur auf produktive (enqueued+dry_run) Aktionen anwenden
    EXIT WHEN (v_enqueued + v_dryrun) >= GREATEST(p_limit, 0);
  END LOOP;

  -- (4) Summary-Audit (immer)
  INSERT INTO public.auto_heal_log(target_id, target_type, action_type, result_status, metadata)
  VALUES (NULL, 'system',
...
-- Grants: nur service_role (Admin-Gate intern via has_role)
...
'Bucket-C Heal: queued Bronze-Pakete mit fehlendem Integrity-Report (score=0, hard_fails=[], approved>=50, kein aktiver integrity-Job) re-enqueuen. Kein Unlock, kein Content-Mutate. Audit: bronze_no_report_reconcile_{dryrun,enqueued,skipped,error,summary}. Default p_dry_run=true.';
