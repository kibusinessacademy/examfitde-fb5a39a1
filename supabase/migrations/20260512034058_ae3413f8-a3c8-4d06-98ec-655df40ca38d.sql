...
BEGIN
  ...
  IF v_prod_with_curr IS NULL OR v_prod_no_curr IS NULL THEN
    RAISE EXCEPTION 'fixtures missing: need active product with curriculum + product without';
  END IF;

  -- 1) synthetic test session — must NOT count
  ...
  -- 2) real paid no grant — MUST count
  INSERT INTO orders(buyer_user_id,status,stripe_checkout_session_id,currency,tax_mode,total_cents)
  ...
  -- 3) real paid WITH grant — must NOT count
  INSERT INTO orders(buyer_user_id,status,stripe_checkout_session_id,currency,tax_mode,total_cents)
  ...
  -- Health (global) — sanity capture
  v_health := public.fn_launch_orders_health();

  -- Health-eligibility restricted to smoke set
...
REVOKE ALL ON FUNCTION public.admin_smoke_launch_orders_health_repair_parity() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_smoke_launch_orders_health_repair_parity() TO authenticated;
