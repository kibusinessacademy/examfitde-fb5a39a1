...
-- 1. Create a DAG edges table as SSOT for step dependencies
CREATE TABLE IF NOT EXISTS public.step_dag_edges (
  step_key text NOT NULL,
  depends_on text NOT NULL,
  PRIMARY KEY (step_key, depends_on)
);

ALTER TABLE public.step_dag_edges ENABLE ROW LEVEL SECURITY;

CREATE POLICY "step_dag_edges_read_all" ON public.step_dag_edges FOR SELECT USING (true);

-- Populate the DAG edges from the known pipeline sequence
INSERT INTO public.step_dag_edges (step_key, depends_on) VALUES
  ...
  ('generate_exam_pool', 'promote_blueprint_variants'),
  ('validate_exam_pool', 'generate_exam_pool'),
  ('repair_exam_pool_quality', 'validate_exam_pool'),
...
-- 2. Replace fn_reconcile_orphan_steps with DAG-aware version
...
BEGIN
  ...
  LOOP
    ...
    -- ═══ DM1: DAG PREDECESSOR CHECK ═══
    -- Block if ANY predecessor step is NOT in terminal state (done/skipped)
    SELECT NOT EXISTS (
      SELECT 1
