...
-- 1) Fix fn_reconcile_orphan_steps to use snake_case payload keys
...
-- 2) Fix existing broken job payloads: add snake_case keys from camelCase
UPDATE job_queue
SET payload = payload
  || jsonb_build_object(
    ...
    last_error = NULL,
    attempts = 0,
    meta = meta - 'last_error_class' - 'last_error_kind' - 'last_error_reason'
WHERE meta->>'source' = 'orphan_reconciler'
