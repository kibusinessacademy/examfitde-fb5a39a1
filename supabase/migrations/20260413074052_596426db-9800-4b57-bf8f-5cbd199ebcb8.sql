...
-- =============================================================
-- ARTIFACT-TRUTH ADOPTION: fn_prebuild_auto_seed_exam_blueprints
-- Short-circuits when ≥10 approved blueprints already exist
-- =============================================================
CREATE OR REPLACE FUNCTION public.fn_prebuild_auto_seed_exam_blueprints(
  p_package_id uuid
)
...
BEGIN
  ...
  -- Count approved blueprints (artifact truth)
  ...
  -- Gate: need at least 10 approved blueprints
  ...
  -- Artifact truth verified → mark step done
...
-- =============================================================
-- ARTIFACT-TRUTH ADOPTION: fn_prebuild_generate_blueprint_variants
-- Short-circuits when variants already exist for blueprints
-- =============================================================
CREATE OR REPLACE FUNCTION public.fn_prebuild_generate_blueprint_variants(
  p_package_id uuid
)
RETURNS TABLE(status text, advanced boolean, reason text, meta jsonb)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Count blueprints that have at least 1 variant
  ...
  -- Coverage: what % of blueprints have variants?
  v_coverage_pct := ROUND((v_blueprints_with_variants::numeric / v_total_blueprints) * 100, 1);

  -- Gate: at least 80% of blueprints must have at least 1 variant
...
-- =============================================================
-- ARTIFACT-TRUTH ADOPTION: fn_prebuild_validate_blueprint_variants
-- Short-circuits when variants exist and have valid data
-- =============================================================
CREATE OR REPLACE FUNCTION public.fn_prebuild_validate_blueprint_variants(
  p_package_id uuid
)
RETURNS TABLE(status text, advanced boolean, reason text, meta jsonb)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Count variants in review or better status
  ...
  -- Gate: need variants, and most should be in review+
