...
BEGIN
  ...
  SELECT * INTO v_run FROM admin_seo_bridge_promotion_execute(
    ...
    p_suggestion_ids := ARRAY[
      ...
      'c92cd5bc-f284-46cc-8dcf-e9c4b4fa5cc8'
    ]::uuid[],
    p_batch_label := 'e3e4_wave2_promote10_dryrun_2026-05-18',
    p_dry_run := true
  );

  RAISE NOTICE 'DRY-RUN run_id=% requested=% promoted=% skipped=% dry=%',
    v_run.run_id, v_run.requested, v_run.promoted, v_run.skipped, v_run.dry_run;
END $$;