...
BEGIN
  ...
  -- Debug: View-Count innerhalb der Funktion
  SELECT COUNT(*) INTO v_view_count
  FROM public.v_quality_gate_blocked_packages
  WHERE minutes_since_report >= 60;

  INSERT INTO public.auto_heal_log(action_type, result_status, target_type, target_id, metadata)
  VALUES ('quality_gate_auto_heal', 'unknown', 'system', 'debug_pre_loop', 
          jsonb_build_object('view_eligible_count', v_view_count, 'enabled', v_enabled));
  ...
  FOR v_pkg IN
    ...
    LIMIT 20
  LOOP
    v_loop_iters := v_loop_iters + 1;
    v_classification := public.fn_classify_quality_gate_block(v_pkg.package_id);

    INSERT INTO public.auto_heal_log(action_type, result_status, target_type, target_id, metadata)
    VALUES ('quality_gate_auto_heal', 'unknown', 'course_package', v_pkg.package_id::text,
            jsonb_build_object('debug', 'iter', 'classification', v_classification));
  ...
  RETURN jsonb_build_object(
    ...
    'healed', v_healed,
    'skipped', v_skipped,
    'loop_iters', v_loop_iters,
    'view_eligible_count', v_view_count,
