...
BEGIN
  ...
  SELECT c.id INTO v_curriculum_id
  ...
  IF v_curriculum_id IS NULL THEN
    RAISE EXCEPTION 'no orphan curriculum_id available for smoke';
  END IF;
  ...
  -- T1: queued-step → not in v_queued_tail_without_job (filter: blocked)
  ...
  -- T2: pending → also excluded
  UPDATE public.package_steps SET status='pending', updated_at = now() - interval '3 hours' WHERE id = v_step_id;
  PERFORM public.admin_reconcile_queued_tail_without_job(p_limit := 5);
  IF NOT EXISTS (SELECT 1 FROM public.job_queue WHERE package_id = v_pkg_id) THEN
    v_passed := v_passed+1; v_results := v_results || jsonb_build_object('test','T2_pending_no_enqueue','pass',true);
  ELSE
    v_failed := v_failed+1; v_results := v_results || jsonb_build_object('test','T2_pending_no_enqueue','pass',false);
  END IF;

  -- T3: blocked → enqueue OK
  ...
  -- T4: second run within 5min → cooldown skip-audit for THIS package
  UPDATE public.package_steps SET status='blocked', updated_at = now() - interval '3 hours' WHERE id = v_step_id;
