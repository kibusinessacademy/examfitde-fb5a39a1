...
-- RPC: seed_blueprint_targets_for_curriculum (idempotent, track-aware)
CREATE OR REPLACE FUNCTION public.seed_blueprint_targets_for_curriculum(
...
BEGIN
  ...
  SELECT COALESCE(
    p_track,
    (SELECT cp.track FROM public.course_packages cp WHERE cp.curriculum_id = p_curriculum_id ORDER BY cp.created_at DESC LIMIT 1),
    'AUSBILDUNG_VOLL'
  ) INTO v_track;

  -- defaults by track + mode
