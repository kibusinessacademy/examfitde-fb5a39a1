...
-- Wire mastery-weighted assembly into start_exam_session for simulation mode
...
BEGIN
  ...
  -- READINESS GATE: only for simulation/timed_exam
  ...
  -- ADAPTIVE MODE: seed only the first question via mastery-weighted picker
  ...
  -- SIMULATION/TIMED/PRACTICE: Use mastery-weighted assembly
  v_mastery_qids := public.assemble_mastery_weighted_exam(
    v_user_id,
    v_blueprint.curriculum_id,
    v_blueprint.total_questions,
    2  -- min 2 questions per LF
  );

  -- If mastery assembly returned questions, use them
  IF v_mastery_qids IS NOT NULL AND array_length(v_mastery_qids, 1) > 0 THEN
    ...
    ORDER BY random();
  ELSE
    -- Fallback: random selection if no mastery data or no skill mappings
    INSERT INTO public.exam_session_questions (
      exam_session_id, question_id, order_index, difficulty,
      learning_field_code, competency_code
    )
