...
-- Adaptive mode support for start_exam_session
-- When mode='adaptive': skip readiness gate, seed only 1 question via pick_next_adaptive_question
...
BEGIN
  ...
  IF p_mode IN ('simulation', 'timed_exam') THEN
    ...
    EXCEPTION WHEN undefined_function THEN
      -- get_exam_readiness may not exist yet, skip gate
      NULL;
    END;
  END IF;
  ...
  IF p_mode = 'adaptive' THEN
    ...
    -- pick_next_adaptive_question already inserts via append, but let's be safe
    -- Check if already inserted by append
    IF NOT EXISTS (
  ...
  -- STANDARD MODES: seed all questions upfront
  INSERT INTO public.exam_session_questions (
    exam_session_id, question_id, order_index, difficulty,
