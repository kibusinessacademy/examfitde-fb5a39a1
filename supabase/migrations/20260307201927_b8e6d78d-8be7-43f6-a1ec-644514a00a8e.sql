...
-- Fix finish_exam_session: use question_skill_map for by_skill_node instead of string matching
...
BEGIN
  ...
  -- by_competency (via competency_code -> competencies -> learning_fields for SSOT)
  FOR rec IN
    SELECT
  ...
  -- by_skill_node via question_skill_map (relational, not string matching)
  FOR rec IN
    SELECT
    ...
    JOIN public.skill_nodes sn ON sn.id = qsm.skill_node_id
    WHERE esq.exam_session_id = p_session_id
