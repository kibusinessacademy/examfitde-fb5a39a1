-- Mirror redundant-seeding suppression to auto_heal_log for single-pane-of-glass auditing.
-- ops_guardrail_events bleibt SSOT — auto_heal_log ist nur Spiegel.
  ...
  IF _is_truth THEN
    ...
    BEGIN
      INSERT INTO public.auto_heal_log(
        ...
        _pkg_id::text,
        'skipped',
        format('Suppressed %s: %s (bp=%s, variants=%s, coverage=%s)',
               NEW.job_type, _reason, _bp_count, _variant_count, round(_coverage, 3)),
        jsonb_build_object(
          'reason', _reason,
          'job_type', NEW.job_type,
          'step_key', _step_key,
