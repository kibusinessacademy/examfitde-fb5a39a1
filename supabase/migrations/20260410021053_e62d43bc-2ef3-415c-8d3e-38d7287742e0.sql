...
DECLARE
  ...
  v_rec record;
BEGIN
  -- ═══ LAYER 1: Track-aware step skipping (existing logic) ═══
  FOR v_rec IN
  ...
  -- ═══ LAYER 2: Reset STALE_LOCK steps (existing logic) ═══
  FOR v_rec IN
  ...
  -- Steps in 'queued' with prereq-related errors that are no longer valid
  FOR v_rec IN
    ...
    WHERE ps.status = 'queued'
      ...
      -- Only clear if all variant/prereq steps for this package are terminal
      AND NOT EXISTS (
  ...
  LOOP
    ...
    v_details := v_details || jsonb_build_object(
      'package_id', v_rec.package_id,
      'step_key', v_rec.step_key,
      'action', 'clear_stale_prereq_error',
      'old_error', left(v_rec.last_error, 100)
    );
  END LOOP;

  -- ═══ LAYER 4 (NEW): Clear stale stuck_reason on healthy packages ═══
  -- Packages with stuck_reason but 0 failed jobs and active work
  FOR v_rec IN
  ...
  LOOP
    ...
    v_details := v_details || jsonb_build_object(
      'package_id', v_rec.package_id,
      'action', 'clear_stale_stuck_reason',
  ...
  IF (v_healed + v_errors_cleared + v_stuck_cleared) > 0 THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'errors_cleared', v_errors_cleared,
        'stuck_cleared', v_stuck_cleared,
        'details', v_details
      )
    );
