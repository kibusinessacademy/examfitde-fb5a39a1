-- Fix lease acquisition order: transition queued/failed package to building BEFORE writing package_leases
-- to satisfy trg_guard_package_leases_building_only and eliminate OPS_GUARD:PACKAGE_LEASES_NON_BUILDING loops.
...
BEGIN
  ...
  FROM public.course_packages
  WHERE priority <= 10
    AND status NOT IN ('published', 'done', 'blocked');
  ...
  -- CRITICAL ORDER FIX: transition first, then lease insert (guard requires building)
