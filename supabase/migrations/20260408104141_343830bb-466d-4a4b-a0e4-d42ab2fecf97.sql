...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Insert as 'review' status to avoid quality guard, with mapped competency where possible
  ...
  -- Now bulk-approve the shared questions (they have valid competency + lf)
  ...
  WHERE status = 'review'
    ...
    AND curriculum_id IN (
      SELECT DISTINCT curriculum_id FROM fi_core_lf_equivalence
    )
