...
DECLARE
  ...
  v_job_count int; v_audit_count int; v_test_label text;
  v_results jsonb := '[]'::jsonb; v_failures int := 0; v_passed int := 0;
...
BEGIN
  ...
  FOR v_test_label IN SELECT unnest(ARRAY['queued','pending','processing','retry_scheduled']) LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      v_results := v_results || jsonb_build_object('test','B1_dup_'||v_test_label||'_lt60s','pass', false, 'skipped_reason', 'status_not_insertable: '||SQLERRM);
    ...
    v_results := v_results || jsonb_build_object('test','B1_dup_'||v_test_label||'_lt60s','pass', v_job_count=0 AND v_audit_count=1, 'jobs',v_job_count,'audits',v_audit_count);
    IF v_job_count=0 AND v_audit_count=1 THEN v_passed:=v_passed+1; ELSE v_failures:=v_failures+1; END IF;
    DELETE FROM job_queue WHERE package_id=v_pkg_id;
  ...
  -- B2 negative
  INSERT INTO job_queue(job_type, payload, status, max_attempts, priority, package_id, created_at, updated_at)
  VALUES (v_job_type, v_payload, 'queued', 8, 50, v_pkg_id,
  ...
  SELECT count(*) INTO v_audit_count FROM auto_heal_log
    WHERE action_type='atomic_enqueue_skipped_recent_duplicate' AND target_id=v_step_id::text;
  v_results := v_results || jsonb_build_object('test','B2_dup_gt60s_negative','pass', v_audit_count=0, 'audits',v_audit_count);
  IF v_audit_count=0 THEN v_passed:=v_passed+1; ELSE v_failures:=v_failures+1; END IF;
...
EXCEPTION WHEN OTHERS THEN
  ...
  VALUES ('phantom_producer_guard_v1_smoke_run', 'system', 'failure',
          jsonb_build_object('error', SQLERRM, 'sqlstate', SQLSTATE,
                             ...
                             'passed', v_passed, 'failed', v_failures));
  RAISE;
END;
$function$;