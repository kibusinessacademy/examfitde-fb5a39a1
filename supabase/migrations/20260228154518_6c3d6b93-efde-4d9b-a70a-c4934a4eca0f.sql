...
-- Sustainable enrichment gate: prevent building packages with <100% enrichment
-- This eliminates the need for manual_freeze entirely
...
DROP TRIGGER IF EXISTS trg_guard_building_requires_enrichment ON course_packages;
CREATE TRIGGER trg_guard_building_requires_enrichment
  BEFORE UPDATE ON course_packages
  FOR EACH ROW
  EXECUTE FUNCTION guard_building_requires_enrichment();
