...
BEGIN
  ...
  FOR rec IN
    ...
    WHERE v.next_tail_step IS NOT NULL
      -- Gate-Class ist SSOT; reconciler_verdict darf BRONZE_REVIEW_TERMINAL sein,
      -- weil die View die neue BRONZE_REVIEW_CLEAN-Klasse nicht kennt.
      AND v.reconciler_verdict IN ('ELIGIBLE','BRONZE_REVIEW_TERMINAL')
      AND (
    ...
    BEGIN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'enqueue_source', 'queued_tail_reconciler_v3_1_gate_aware',
      ...
      VALUES (rec.package_id::text, 'package', 'queued_tail_reconciler_enqueue', 'success',
              jsonb_build_object('package_id', rec.package_id,
                                 ...
                                 'rpc_version','v3_1_verdict_override'));
...
VALUES (
  'queued_tail_reconciler_v3_1_deployed', 'system', NULL, 'success',
  jsonb_build_object(
    'change', 'reconciler_verdict accepts ELIGIBLE OR BRONZE_REVIEW_TERMINAL when gate_class is in allowlist',
    'rationale', 'View kennt neue BRONZE_REVIEW_CLEAN-Klasse nicht; Gate-Class ist SSOT'
  )
);
