  ...
  try {
    ...
    for (const bpId of blueprintIds) {
      ...
      const { data: bp, error: bpErr } = await sb
        .from("question_blueprints")
        .select("name, curriculum_id, learning_field_id, competency_id, knowledge_type, cognitive_level, rubric, trap_definition")
        .eq("id", bpId)
        .single();
      ...
      for (const v of variants.slice(0, remaining)) {
        ...
        toInsert.push({
          ...
          is_trap: isTrap,
          trap_type: sanitizeTrapType(v.trap_type),
          trap_tags: sanitizeTrapType(v.trap_type) ? [sanitizeTrapType(v.trap_type)!] : [],
