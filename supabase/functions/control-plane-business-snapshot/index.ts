...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const activeCurricula = Number(curriculaRes.count || 0);
    const monetizedCurricula = Number(monetizedRes.count || 0);
    const activeCampaigns = Number(campaignsRes.count || 0);
    const activeWaves = Number(wavesActiveRes.count || 0);
    const blockedWaves = Number(wavesBlockedRes.count || 0);
    ...
    const { data, error } = await sb
      ...
      .insert({
        ...
        monetized_curricula: monetizedCurricula,
