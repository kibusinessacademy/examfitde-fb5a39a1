/**
 * stuck-scan: Orphan processing, enqueued-drift, status-lag, and
 * batch-complete-stuck self-heal.
 *
...
export async function healOrphanProcessing(sb: SupabaseClient) {
  ...
  for (const ps of processingSteps || []) {
    ...
    // ── SSOT: Use filterGenuinelyActiveJobs instead of raw count ──
    ...
    await sb.from("package_steps").update({
      ...
      meta: {
        ...
        last_progress_note: `orphan-heal: processing>${ageMin}min, no genuinely active jobs`,
    ...
    await sb.from("auto_heal_log").insert({
      ...
      result_detail: `Step ${ps.step_key} was processing for ${ageMin}min with 0 genuinely active jobs — reset to queued (attempt ${newAttempts}, stall ${stallCount})`,
    ...
    console.warn(`[stuck-scan] 🧟‍♂️ Orphan-heal: ${ps.step_key} for ${ps.package_id.slice(0,8)} — processing ${ageMin}min, no genuinely active jobs → queued`);
  }
...
export async function healEnqueuedDrift(sb: SupabaseClient) {
  ...
  for (const ps of enqSteps || []) {
    ...
    // ── SSOT: Use filterGenuinelyActiveJobs instead of raw count ──
    const { data: activeJobs, error: jobErr } = await sb
      .from("job_queue")
      .select("id, status, attempts, last_error")
      .eq("package_id", ps.package_id)
      .eq("job_type", jobType)
    ...
    await sb.from("package_steps").update({
      ...
      meta: {
        ...prevMeta,
        last_progress_note: `enqueued-drift-heal: no genuinely active job for ${ageMin}min`,
    ...
    await sb.from("auto_heal_log").insert({
      ...
      target_type: "package_step", target_id: ps.package_id, result_status: "applied",
      result_detail: `Step ${ps.step_key} enqueued ${ageMin}min with 0 genuinely active jobs — reset to queued`,
    ...
    console.warn(`[stuck-scan] 📭 Enqueued-drift-heal: ${ps.step_key} for ${ps.package_id.slice(0,8)} — enqueued ${ageMin}min, no genuinely active jobs → queued`);
  }
 ...
 * completion (batch_complete=true OR ok=true) and no genuinely active jobs remain.
 * Transition them to "done" via markStepDone (SSOT postcondition gate).
 *
 * Uses SSOT isStepFinalizable() — the central finalizability check.
 */
export async function healBatchCompleteStuck(sb: SupabaseClient) {
  ...
  const { data: stuckSteps } = await sb
    .from("package_steps")
    .select("package_id, step_key, status, updated_at, started_at, meta, attempts")
  ...
  for (const ps of stuckSteps || []) {
    ...
    }, jobType, { minAgeMs: META_COMPLETE_MIN_AGE_MS });

    if (!finResult.finalizable) continue;

    // Cancel terminal-loop jobs before finalization
    if (finResult.terminalJobs > 0 && jobType) {
      const cancelled = await cancelTerminalLoopJobs(sb, ps.package_id, jobType);
      if (cancelled > 0) {
        console.log(`[stuck-scan] 🔓 Meta-complete-heal: cancelled ${cancelled} terminal-loop job(s) for ${ps.step_key} (${ps.package_id.slice(0, 8)})`);
      }
    }

    const ageMs = Date.now() - new Date(ps.updated_at).getTime();
    const ageMin = Math.round(ageMs / 60000);
    const signal = meta.batch_complete === true ? "batch_complete" : "meta.ok";
    ...
    try {
      ...
      // Stale meta cleanup: clear false completion signals
