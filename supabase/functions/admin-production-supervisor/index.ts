...
/**
 * admin-production-supervisor — Global production orchestrator.
 *
 * Actions:
 *   "status"     — Returns wave summary + global health
 *   "activate"   — Starts a wave (sets items to queued, packages to queued)
 *   "tick"       — Supervisor tick: syncs item status from packages, advances wave
 *   "pause"      — Pauses a wave (stops new package starts)
 *   "resume"     — Resumes a paused wave
 *   "finalize"   — Marks wave as completed, generates report
 */
...
Deno.serve(async (req) => {
  ...
  // ═══════════════════════════════════════════════════════════════
  // ACTION: activate — Start the wave
  // ═══════════════════════════════════════════════════════════════
  if (action === "activate") {
    if (wave.status !== "draft" && wave.status !== "paused") {
      return json(400, { error: `Cannot activate wave in status: ${wave.status}` }, origin);
    }

    // Set pending items to queued
    ...
    // Activate packages (set to queued with appropriate priority)
    let activated = 0;
  ...
  // ═══════════════════════════════════════════════════════════════
  // ACTION: tick — Sync item status from actual package state
  // ═══════════════════════════════════════════════════════════════
  if (action === "tick") {
    const { data: items } = await sb
      ...
      .eq("wave_id", waveId)
      .not("status", "in", "(published,blocked,skipped)");
    ...
    for (const item of items || []) {
      ...
      if (pkg.published_at || pkg.status === "published") {
        ...
        // Check if it's a quality gate failure or a build failure
      ...
      } else if (pkg.status === "draft" && pkg.build_progress >= 100) {
    ...
    // Auto-finalize if all items terminal
    const totalItems = Object.values(itemCounts).reduce((a: number, b: any) => a + (Number(b) || 0), 0);
    const terminalItems = (itemCounts.published ?? 0) + (itemCounts.blocked ?? 0) +
      (itemCounts.quality_gate_passed ?? 0) + (itemCounts.quality_gate_failed ?? 0) + (itemCounts.skipped ?? 0);
  ...
  // ═══════════════════════════════════════════════════════════════
  // ACTION: finalize — Generate wave report
  // ═══════════════════════════════════════════════════════════════
  if (action === "finalize") {
    ...
    const report = {
      ...
      items: (allItems || []).map((i: any) => ({
        ...
        status: i.status,
        duration_min: i.started_at && i.finished_at
          ? Math.round((new Date(i.finished_at).getTime() - new Date(i.started_at).getTime()) / 60000)
          : null,
