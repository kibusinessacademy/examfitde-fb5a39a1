...
export const BLOOM_TAXONOMY = {
  remember: {
    level: 1, code: 'K1', name_de: 'Erinnern', name_en: 'Remember', ihk_weight: 0.10,
    ...
    question_stems_de: ['Was ist...?', 'Nennen Sie...', 'Welche... gibt es?', 'Definieren Sie...'],
    spaced_rep_modifier: 1.0,
  },
  understand: {
    level: 2, code: 'K2', name_de: 'Verstehen', name_en: 'Understand', ihk_weight: 0.25,
    ...
    question_stems_de: ['Erklären Sie...', 'Was bedeutet...?', 'Warum ist...?', 'Beschreiben Sie den Unterschied zwischen...'],
    spaced_rep_modifier: 1.05,
  },
  apply: {
    level: 3, code: 'K3', name_de: 'Anwenden', name_en: 'Apply', ihk_weight: 0.35,
    ...
    question_stems_de: ['Wie würden Sie...?', 'Berechnen Sie...', 'Führen Sie... durch', 'Wenden Sie... an auf...'],
    spaced_rep_modifier: 1.10,
  },
  analyze: {
    level: 4, code: 'K4', name_de: 'Analysieren', name_en: 'Analyze', ihk_weight: 0.20,
    ...
    question_stems_de: ['Analysieren Sie...', 'Welche Zusammenhänge bestehen zwischen...?', 'Vergleichen Sie...', 'Welche Faktoren beeinflussen...?'],
    spaced_rep_modifier: 1.15,
  },
  evaluate: {
    level: 5, code: 'K5', name_de: 'Bewerten', name_en: 'Evaluate', ihk_weight: 0.07,
    ...
    question_stems_de: ['Bewerten Sie...', 'Welche Vor- und Nachteile hat...?', 'Wie beurteilen Sie...?', 'Was empfehlen Sie und warum?'],
    spaced_rep_modifier: 1.20,
  },
  create: {
    level: 6, code: 'K6', name_de: 'Erschaffen', name_en: 'Create', ihk_weight: 0.03,
    ...
    question_stems_de: ['Entwickeln Sie...', 'Entwerfen Sie...', 'Erstellen Sie einen Plan für...', 'Wie würden Sie... gestalten?'],
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let result;
    switch (action) {
      case 'get_taxonomy': result = getTaxonomy(); break;
      case 'classify_question': result = await classifyQuestion(params); break;
      case 'get_ihk_distribution': result = await getIHKDistribution(supabase, params); break;
      case 'get_level_stats': result = await getLevelStats(supabase, params); break;
      case 'generate_question_for_level': result = await generateQuestionForLevel(supabase, params); break;
...
function getTaxonomy() {
  return {
    ...
    levels: Object.entries(BLOOM_TAXONOMY).map(([key, value]) => ({ key, ...value })),
  ...
  try {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        const level = parsed.level as BloomLevel;
        if (BLOOM_TAXONOMY[level]) {
          return { level, bloom_data: BLOOM_TAXONOMY[level], confidence: parsed.confidence || 0.8, reasoning: parsed.reasoning || 'AI-Klassifikation' };
        }
      }
    } catch { /* fall through */ }
  } catch { /* fall through */ }
...
async function getIHKDistribution(supabase: any, params: any) {
  ...
  const { data: questions } = await supabase
    .from('exam_questions').select(`id, difficulty, blueprint_variants!left(blueprint:question_blueprints(cognitive_level))`)
    .eq('curriculum_id', curriculum_id).eq('status', 'approved');
  ...
  Object.entries(BLOOM_TAXONOMY).forEach(([level, data]) => {
    distribution[level] = { count: 0, target: Math.round(totalQuestions * data.ihk_weight), ihk_weight: data.ihk_weight };
  });
  questions?.forEach((q: any) => {
    const level = q.blueprint_variants?.[0]?.blueprint?.cognitive_level || mapDifficultyToBloom(q.difficulty);
    if (distribution[level]) distribution[level].count++;
  });
  const gaps = Object.entries(distribution).map(([level, data]) => ({
    level, level_data: BLOOM_TAXONOMY[level as BloomLevel], count: data.count, target: data.target,
  ...
  return { total_questions: totalQuestions, distribution: gaps, is_balanced: gaps.every(g => Math.abs(g.gap) <= 2) };
}

async function getLevelStats(supabase: any, params: any) {
  const { data } = await supabase.rpc('get_bloom_level_stats', { p_curriculum_id: params.curriculum_id });
  return { stats: data || [], taxonomy: BLOOM_TAXONOMY };
}

async function generateQuestionForLevel(supabase: any, params: any) {
  const { bloom_level, competency_id } = params;
  const levelData = BLOOM_TAXONOMY[bloom_level as BloomLevel];
  if (!levelData) throw new Error(`Invalid Bloom level: ${bloom_level}`);
  let competencyContext = '';
  if (competency_id) {
    const { data: competency } = await supabase.from('competencies').select('title, code, learning_field:learning_fields(title)').eq('id', competency_id).single();
    if (competency) competencyContext = `\nLernfeld: ${competency.learning_field?.title}\nKompetenz: ${competency.title} (${competency.code})`;
  }
  return {
    bloom_level, level_data: levelData, question_stems: levelData.question_stems_de, action_verbs: levelData.verbs_de,
...
function mapDifficultyToBloom(difficulty: string): BloomLevel {
  switch (difficulty) { case 'easy': return 'remember'; case 'medium': return 'understand'; case 'hard': return 'apply'; default: return 'understand'; }
}
