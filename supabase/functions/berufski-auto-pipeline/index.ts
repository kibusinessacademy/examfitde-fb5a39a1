...
async function invokeFunction(
  ...
  if (!res.ok) {
    return { data, error: data?.error || `${fnName} returned ${res.status}` };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (!berufskiId || !tier) {
      return new Response(
        JSON.stringify({ error: "berufskiId and tier required" }),
    ...
    const gen = await invokeFunction(supabaseUrl, serviceKey, "berufski-generate-product", {
      berufskiId,
      tier,
    });
    if (gen.error) {
    ...
    // ─── Step 2: Render Screen PDF ───
    log("Step 2: Render screen PDF");
    const screenPdf = await invokeFunction(supabaseUrl, serviceKey, "berufski-render-pdf", {
      productId,
      mode: "screen",
    });
    steps.push({
      step: "render_screen",
      status: screenPdf.error ? "failed" : "ok",
      detail: screenPdf.error || { path: screenPdf.data?.storagePath },
    });

    // ─── Step 3: Render Print PDF ───
    log("Step 3: Render print PDF");
    const printPdf = await invokeFunction(supabaseUrl, serviceKey, "berufski-render-pdf", {
      productId,
      mode: "print",
    });
    steps.push({
      step: "render_print",
      status: printPdf.error ? "failed" : "ok",
      detail: printPdf.error || { path: printPdf.data?.storagePath },
    });

    // ─── Step 4: Publish (Stripe + status) ───
    log("Step 4: Publish gate");
    const pub = await invokeFunction(supabaseUrl, serviceKey, "berufski-publish-gate", {
      productId,
    });
    steps.push({
      step: "publish",
      status: pub.error ? "failed" : "ok",
      detail: pub.error || { stripePriceId: pub.data?.stripePriceId },
    });

    // ─── Step 5: Build landing URL ───
    const { data: produkt } = await adminClient
    ...
    if (produkt?.beruf_id) {
      const { data: beruf } = await adminClient
        ...
        .select("slug")
        .eq("id", produkt.beruf_id)
        .single();
      if (beruf?.slug) {
        const appBase = Deno.env.get("APP_BASE_URL") || "https://examfit.de";
        landingUrl = `${appBase}/berufski/ki-fuer/${beruf.slug}`;
      }
    }
