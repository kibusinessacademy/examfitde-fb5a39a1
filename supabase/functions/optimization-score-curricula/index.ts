...
Deno.serve(async (req) => {
  ...
  const { data: run } = await sb
    ...
    .insert({ run_type: "curriculum_score", status: "running" })
    .select("id")
    .single();
  const runId = run?.id;
  ...
  for (const row of rows || []) {
    const { data, error: rpcErr } = await sb.rpc("compute_curriculum_scaling_signal", {
      p_qualification_catalog_id: row.id,
    });
    if (rpcErr) { errorCount++; results.push({ id: row.id, error: rpcErr.message }); }
