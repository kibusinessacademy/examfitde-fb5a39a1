/**
 * build-standalone-snapshot — DEPRECATED 2026-05-17
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (url && key) {
      ...
      await admin.rpc("fn_emit_audit", {
        ...
        _target_id: "build-standalone-snapshot",
        _result_status: "warning",
        _payload: {
          fn: "build-standalone-snapshot",
          method: req.method,
  ...
  return new Response(
    JSON.stringify({
      ok: false,
      error: "gone",
      message: "build-standalone-snapshot wurde am 2026-05-17 deprecated.",
    }),
