...
export function PublishReadinessCard() {
  const { data, isLoading, error } = usePublishReadiness();

  if (isLoading) return <LoadingState label="Lade Publish Readiness…" />;
  if (error) return <ErrorState label="Publish Readiness konnte nicht geladen werden." />;
        ...
        <table className="w-full min-w-[1200px] text-xs">
          <thead>
            <tr className="border-b text-left text-muted-foreground">
              ...
              <th className="py-2 pr-2">Integrity</th>
              <th className="py-2 pr-2">QC</th>
              <th className="py-2 pr-2">Publish</th>
              ...
              <th className="py-2 text-right">Tutor</th>
            </tr>
          </thead>
          <tbody>
            {data?.map((row: any) => {
              const dot = row.publish_ready ? "green" : row.primary_blocker === "QUALITY_COUNCIL_PENDING" ? "yellow" : "red";
              return (
                <tr key={row.package_id} className="border-b border-border/50 align-top hover:bg-muted/30">
                  ...
                  <td className="py-2 pr-2">{row.publish_ready ? "✅" : "❌"}</td>
                  <td className="py-2 pr-3 text-destructive text-[10px]">{row.primary_blocker ?? "—"}</td>
                  <td className="py-2 pr-2">{row.integrity_passed ? "✅" : "❌"}</td>
                  <td className="py-2 pr-2 text-muted-foreground">{row.quality_council_status ?? "—"}</td>
                  <td className="py-2 pr-2 text-muted-foreground">{row.auto_publish_status ?? "—"}</td>
                  ...
                  <td className="py-2 text-right tabular-nums">{row.tutor_index_items}</td>
                </tr>
              );
            })}
          </tbody>
