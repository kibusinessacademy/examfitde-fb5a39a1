      ...
      <CardContent className="py-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px]">
          <div>
            ...
            <span className={cn("ml-1 font-mono", errRate > 10 ? 'text-destructive' : 'text-success')}>
              {100 - errRate}%
            </span>
          </div>
          <div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {latest ? (
          ...
          <Badge variant="outline" className="text-xs text-muted-foreground">Keine Daten</Badge>
        )}
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <Card>
            <CardContent className="py-3 text-center">
              ...
              <p className={cn("text-xl font-bold",
                latest.low_confidence_count > 10 ? 'text-warning' : 'text-foreground'
              )}>{latest.low_confidence_count}</p>
      ...
      {latest && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Schwierigkeitsverteilung</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex justify-between mt-1 text-[10px] text-muted-foreground">
              <span>Leicht (Ziel: 40%)</span>
              <span>Mittel (Ziel: 40%)</span>
              <span>Schwer (Ziel: 20%)</span>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {latest?.lf_detail && (
        <Card>
          ...
          <CardContent>
            <LFHeatmap detail={latest.lf_detail} total={latest.total_questions} />
          </CardContent>
        </Card>
      )}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {providers.slice(0, 6).map(p => (
              <ProviderCard key={`${p.provider}-${(p as any).date}`} p={p} />
            ))}
          </div>
        </div>
      )}
      ...
      {allFlags.length > 0 && (
        <Card className="border-warning/50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 text-warning" /> Aktive Flags
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {allFlags.map(f => (
                <Badge key={f} variant="outline" className="text-xs border-warning/50 text-warning">
                  {f.replace(/_/g, ' ')}
                </Badge>
              ))}
            </div>
      ...
      {snapshots.length > 1 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Snapshot-Verlauf</CardTitle>
          </CardHeader>
          <CardContent>
