...
const SECONDARY_ITEMS = [
  ...
  { to: '/admin/berufs-ki/graph', label: 'Berufs-KI Graph', icon: Sparkles },
        ...
        <div className="px-2 mt-2 space-y-0.5 border-t border-border pt-2">
          ...
          {SECONDARY_ITEMS.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) => cn(
                "flex items-center gap-3 rounded-xl px-3 py-2 text-xs font-medium transition-colors min-h-[40px]",
                isActive
                  ? "bg-primary/10 text-primary"
                  : "text-muted-foreground hover:bg-muted hover:text-foreground"
              )}
