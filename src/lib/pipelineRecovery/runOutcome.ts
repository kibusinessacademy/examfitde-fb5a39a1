/**
 * PIPELINE.RECOVERY.OS.2 — Outcome verification (pure SSOT).
 * Deterministic, side-effect free. Classifies the post-state of a single
 * recovery action vs its pre-state. NEVER mutates publish/integrity fields.
 */
import type { RecoveryActionType } from "./contracts";
...
const VERIFICATION_GRACE_MS = 60_000; // post-state must be observed >= 60s after action

/** Deterministic classification. Never touches publish/integrity fields. */
export function classifyOutcome(
  ...
  switch (actionType) {
    case "enqueue_done_reaudit":
      return classifyReaudit(probe);
    case "restart_planning":
      return classifyRestart(probe);
    case "mark_manual_review_required":
      return classifyManualReview(probe);
    case "propose_provider_fallback":
      return { status: "verified_success", reason: "proposal_recorded", signals: {} };
    case "diagnose_only":
      return { status: "skipped", reason: "diagnose_only", signals: {} };
    default:
      return { status: "pending_verification", reason: "unknown_action", signals: {} };
  }
}

function classifyReaudit(p: OutcomeProbe): OutcomeVerdict {
  const before = p.jobs_before.filter((j) =>
    j.job_type.startsWith("package_run_integrity") || j.job_type.startsWith("package_quality_council"),
  );
  const after = p.jobs_after.filter((j) =>
    j.job_type.startsWith("package_run_integrity") || j.job_type.startsWith("package_quality_council"),
  );
  ...
  if (newPending > 0) {
    return { status: "pending_verification", reason: "audit_jobs_running", signals: { newPending } };
  }
...
function classifyManualReview(p: OutcomeProbe): OutcomeVerdict {
  if (p.quarantined_after) {
    return { status: "verified_success", reason: "quarantine_recorded", signals: {} };
  }
...
export const RECOVERY_RUN_POLICY = {
  ...
  VERIFICATION_TIMEOUT_MS: 30 * 60 * 1000, // 30 min
