...
export default function WorkBundleBuyPage() {
  const [email, setEmail] = useState('');
  const [coupon, setCoupon] = useState('');
  const [affiliate, setAffiliate] = useState('');
  const [loading, setLoading] = useState(false);

  const bundleId = window.location.pathname.split('/').pop() || new URLSearchParams(window.location.search).get('bundleId') || '';
  ...
  const startCheckout = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('berufski-bundle-checkout', {
        body: { bundleId, buyerEmail: email, couponCode: coupon || null, affiliateCode: affiliate || null, landingPath: window.location.pathname },
      });
  ...
  return (
    <main className="mx-auto max-w-xl p-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">{BRAND.name} Bundle kaufen</CardTitle>
          <CardDescription>Du erhältst den Bundle-Download per E-Mail (PDF / optional ZIP).</CardDescription>
        </CardHeader>
