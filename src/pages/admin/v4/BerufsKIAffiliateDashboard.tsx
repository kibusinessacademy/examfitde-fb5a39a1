...
const BerufsKIAffiliateDashboard = () => {
  ...
  // Product purchases with affiliate codes
  const { data: productSales } = useQuery({
    queryKey: ['berufski-affiliate-product-sales'],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  // Bundle purchases with affiliate codes
  const { data: bundleSales } = useQuery({
    queryKey: ['berufski-affiliate-bundle-sales'],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const { data: clicks } = useQuery({
    queryKey: ['berufski-affiliate-clicks'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('berufski_affiliate_clicks')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(200);
      if (error) throw error;
  ...
  const affiliateSummary = (affiliates || []).map(aff => {
    ...
    return {
      ...aff,
      productOrders: pSales.length,
      bundleOrders: bSales.length,
  ...
  const exportCsv = () => {
    ...
    const rows = affiliateSummary.map(a => [
      a.code, a.name, a.payout_percent, a.clicks, a.productOrders, a.bundleOrders,
      (a.totalRevenue / 100).toFixed(2), (a.commission / 100).toFixed(2),
    ]);
    ...
    a.href = url;
    a.download = `berufski-affiliates-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => syncMutation.mutate()} disabled={syncMutation.isPending}>
            <RefreshCw className="h-4 w-4 mr-1" /> Corp Sync
          </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground"><Users className="h-4 w-4" /> Affiliates</div>
            <div className="text-2xl font-bold mt-1">{affiliates?.length || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground"><TrendingUp className="h-4 w-4" /> Clicks</div>
            <div className="text-2xl font-bold mt-1">{clicks?.length || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground"><DollarSign className="h-4 w-4" /> Umsatz</div>
            <div className="text-2xl font-bold mt-1">{(totalRevAll / 100).toFixed(2)} €</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground"><DollarSign className="h-4 w-4" /> Provision</div>
            <div className="text-2xl font-bold mt-1">{(totalCommAll / 100).toFixed(2)} €</div>
          </CardContent>
        </Card>
      </div>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {affiliateSummary.length === 0 && (
                    <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground">Keine Affiliates</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </CardContent>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {(productSales || []).map(s => (
                    <TableRow key={s.id}>
                      <TableCell><Badge variant="outline">Produkt</Badge></TableCell>
                      <TableCell className="text-sm">{s.user_email}</TableCell>
                      <TableCell className="font-mono text-sm">{s.affiliate_code}</TableCell>
                      <TableCell>{((s.amount_cents || 0) / 100).toFixed(2)} €</TableCell>
                      <TableCell className="text-sm">{new Date(s.created_at).toLocaleDateString('de-DE')}</TableCell>
                    </TableRow>
                  ))}
                  {(bundleSales || []).map(s => (
                    <TableRow key={s.id}>
                      <TableCell><Badge variant="outline">Bundle</Badge></TableCell>
                      <TableCell className="text-sm">{s.user_email}</TableCell>
                      <TableCell className="font-mono text-sm">{s.affiliate_code}</TableCell>
                      <TableCell>{((s.amount_paid_cents || 0) / 100).toFixed(2)} €</TableCell>
                      <TableCell className="text-sm">{new Date(s.created_at).toLocaleDateString('de-DE')}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {(clicks || []).map(c => (
                    <TableRow key={c.id}>
                      <TableCell className="font-mono text-sm">{c.affiliate_code}</TableCell>
                      <TableCell className="text-sm">{c.landing_path}</TableCell>
                      <TableCell className="text-sm truncate max-w-[200px]">{c.referrer || '—'}</TableCell>
