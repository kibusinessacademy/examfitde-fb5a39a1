...
function useOrganizations() {
  return useQuery({
    queryKey: ['work-organizations'],
    queryFn: async () => {
      const { data, error } = await supabase.from('work_organizations').select('*').order('name');
...
function useLicenses(orgId?: string) {
  return useQuery({
    queryKey: ['work-licenses', orgId],
    queryFn: async () => {
      let q = supabase.from('work_licenses').select('*').order('created_at', { ascending: false });
...
function useLicenseKeys(licenseId?: string) {
  return useQuery({
    queryKey: ['work-license-keys', licenseId],
    queryFn: async () => {
      if (!licenseId) return [];
      const { data, error } = await supabase.from('work_license_keys').select('*').eq('license_id', licenseId).order('created_at');
      if (error) throw error;
      return (data || []) as any[];
    },
    enabled: !!licenseId,
...
function OrganizationsSection() {
  ...
  const deleteOrg = useMutation({
    ...
    onSuccess: () => { toast.success('Organisation gelöscht'); qc.invalidateQueries({ queryKey: ['work-organizations'] }); },
              ...
              <CardContent className="py-4 flex items-center justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Building2 className="h-4 w-4" /><span className="font-medium">{o.name}</span>
                    {o.domain && <Badge variant="outline">{o.domain}</Badge>}
                  </div>
                  <p className="text-sm text-muted-foreground">{o.billing_email}{o.vat_id && ` · USt-ID: ${o.vat_id}`}</p>
                </div>
                <Button variant="ghost" size="sm" onClick={() => deleteOrg.mutate(o.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
              </CardContent>
            </Card>
          ))}
        </div>
...
function OrgForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('work_organizations').insert({
        name: form.name, billing_email: form.billing_email,
        domain: form.domain || null, vat_id: form.vat_id || null,
      });
      if (error) throw error;
    },
    onSuccess: () => { toast.success('Organisation erstellt'); qc.invalidateQueries({ queryKey: ['work-organizations'] }); onSuccess(); },
    onError: (e) => toast.error((e as Error).message),
  });
  return (
    <div className="space-y-4">
      <div><Label>Name *</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Mustermann GmbH" /></div>
      <div><Label>Rechnungs-E-Mail *</Label><Input type="email" value={form.billing_email} onChange={e => setForm(f => ({ ...f, billing_email: e.target.value }))} /></div>
      <div className="grid grid-cols-2 gap-4">
        <div><Label>Domain</Label><Input value={form.domain} onChange={e => setForm(f => ({ ...f, domain: e.target.value }))} /></div>
        <div><Label>USt-ID</Label><Input value={form.vat_id} onChange={e => setForm(f => ({ ...f, vat_id: e.target.value }))} /></div>
      </div>
      <Button onClick={() => save.mutate()} disabled={!form.name || !form.billing_email || save.isPending} className="w-full">
        {save.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Erstellen
      </Button>
    </div>
  );
}
...
function LicensesSection() {
  ...
  const statusColors: Record<string, 'default' | 'secondary' | 'destructive'> = { active: 'default', expired: 'destructive', revoked: 'destructive', pending: 'secondary' };
          ...
          {licenses.map((l: any) => (
            <Card key={l.id}>
              <CardContent className="py-4 flex items-center justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Shield className="h-4 w-4" /><span className="font-medium">{orgMap.get(l.org_id) || 'Unbekannt'}</span>
                    <Badge variant={statusColors[l.status || 'pending'] || 'secondary'}>{l.status || 'pending'}</Badge>
                    <Badge variant="outline">{l.plan}</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    <Users className="inline h-3 w-3 mr-1" />{l.seats} Seats · {new Date(l.starts_at).toLocaleDateString('de-DE')} – {new Date(l.ends_at).toLocaleDateString('de-DE')}
                  </p>
                </div>
...
function LicenseForm({ orgs, onSuccess }: { orgs: any[]; onSuccess: () => void }) {
  const qc = useQueryClient();
  const now = new Date();
  const oneYear = new Date(now.getFullYear() + 1, now.getMonth(), now.getDate());
  const [form, setForm] = useState({
    org_id: '', plan: 'team', seats: 10,
    starts_at: now.toISOString().split('T')[0], ends_at: oneYear.toISOString().split('T')[0], watermark_text: '',
  });
  const save = useMutation({
    mutationFn: async () => {
      const org = orgs.find((o: any) => o.id === form.org_id);
      const { error } = await supabase.from('work_licenses').insert({
        ...
        status: 'active', watermark_text: form.watermark_text || `Lizenziert für ${org?.name || ''}`,
      });
      if (error) throw error;
    },
    onSuccess: () => { toast.success('Lizenz erstellt'); qc.invalidateQueries({ queryKey: ['work-licenses'] }); onSuccess(); },
    onError: (e) => toast.error((e as Error).message),
  });
  return (
    <div className="space-y-4">
      ...
      <div><Label>Wasserzeichen-Text</Label><Input value={form.watermark_text} onChange={e => setForm(f => ({ ...f, watermark_text: e.target.value }))} /></div>
      <Button onClick={() => save.mutate()} disabled={!form.org_id || save.isPending} className="w-full">
        {save.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Lizenz erstellen
      </Button>
    </div>
  );
}
...
function KeysSection() {
  ...
  const generateKey = useMutation({
    mutationFn: async (licenseId: string) => {
      const key = `EFW-${crypto.randomUUID().replace(/-/g, '').slice(0, 20).toUpperCase()}`;
      const { error } = await supabase.from('work_license_keys').insert({ license_id: licenseId, key, status: 'active' });
      if (error) throw error;
      return key;
    },
    onSuccess: (key) => { toast.success(`Schlüssel generiert: ${key}`); qc.invalidateQueries({ queryKey: ['work-license-keys'] }); },
    onError: (e) => toast.error((e as Error).message),
  });

  const copyKey = (key: string) => { navigator.clipboard.writeText(key); toast.success('Schlüssel kopiert'); };
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          ...
          <Select value={selectedLicense} onValueChange={setSelectedLicense}>
            ...
            <SelectContent>
              {licenses.map((l: any) => <SelectItem key={l.id} value={l.id}>{l.plan} · {l.seats} Seats · {l.id.slice(0, 8)}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        {selectedLicense && (
          <Button onClick={() => generateKey.mutate(selectedLicense)} disabled={generateKey.isPending} className="mt-6">
            {generateKey.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Key className="mr-2 h-4 w-4" />}Key generieren
          </Button>
        )}
      </div>
      {!selectedLicense ? (
            ...
            <Card key={k.id}>
              <CardContent className="py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => copyKey(k.key)}><Copy className="h-4 w-4" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
